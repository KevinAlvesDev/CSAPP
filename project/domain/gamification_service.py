...
def _calculate_user_gamification_score(
    ...
    # Carrega os valores das regras do DB, com um fallback (default)
    ...
    max_reclamacoes = regras_db.get('eleg_reclamacoes_max', 1) # (User quer 2, então o max permitido é 1)
    max_perda_prazo = regras_db.get('eleg_perda_prazo_max', 2) # (User quer 3, então o max permitido é 2)
    max_nao_preenchimento = regras_db.get('eleg_nao_preenchimento_max', 2) # (User quer 3, então o max permitido é 2)
    ...
    if nq is not None and nq < 80: # Regra fixa que elimina (Nota de Qualidade < 80%)
    ...
    if elegivel:
        # Satisfação (Lógica por Faixa, conforme solicitado)
        ...
        if satisfacao is not None:
            ...
            elif satisfacao >= 95: pts_satisfacao = regras_db.get('pts_satisfacao_95', 17) # 95-99
            elif satisfacao >= 90: pts_satisfacao = regras_db.get('pts_satisfacao_90', 15) # 90-94
            elif satisfacao >= 85: pts_satisfacao = regras_db.get('pts_satisfacao_85', 14) # 85-89
            elif satisfacao >= 80: pts_satisfacao = regras_db.get('pts_satisfacao_80', 12) # 80-84
        ...
        # TMA (Lógica por Faixa, conforme solicitado)
        pts_tma = 0
        tma_display = 'N/A'
        if tma_medio is not None:
            tma_display = f"{tma_medio:.1f} dias"
            if tma_medio <= 30: pts_tma = regras_db.get('pts_tma_30', 45)       # (25-30 dias)
            elif tma_medio <= 35: pts_tma = regras_db.get('pts_tma_35', 32)    # (31-35 dias)
            elif tma_medio <= 40: pts_tma = regras_db.get('pts_tma_40', 24)    # (36-40 dias)
            elif tma_medio <= 45: pts_tma = regras_db.get('pts_tma_45', 16)    # (41-45 dias)
            else: pts_tma = regras_db.get('pts_tma_46_mais', 8)                 # (>= 46 dias)
        pontos += pts_tma
        ...
        # Planos de Sucesso (Lógica por Faixa)
        pts_planos = 0
        if psp is not None:
        ...
        # Impl. Iniciadas
        pts_iniciadas = 0
        if count_iniciadas >= 10: pts_iniciadas = regras_db.get('pts_iniciadas_10', 25)
        elif count_iniciadas >= 9: pts_iniciadas = regras_db.get('pts_iniciadas_9', 20)
        elif count_iniciadas >= 8: pts_iniciadas = regras_db.get('pts_iniciadas_8', 18)
        elif count_iniciadas >= 7: pts_iniciadas = regras_db.get('pts_iniciadas_7', 14)
        elif count_iniciadas >= 6: pts_iniciadas = regras_db.get('pts_iniciadas_6', 10)
        pontos += pts_iniciadas
        detalhamento_pontos['Impl. Iniciadas'] = f"{pts_iniciadas} pts ({count_iniciadas})"

        # Qualidade (Lógica por Faixa)
        pts_qualidade = 0
        if nq is not None:
        ...
        pts_bonus_elogios = min(elogios, 1) * regras_db.get('bonus_elogios', 15) # Limite de 1 elogio
        pts_bonus += pts_bonus_elogios
        if elogios > 0: detalhamento_pontos['Bônus Elogios'] = f"+{pts_bonus_elogios} pts ({elogios} ocorr.)"
        ...
        if recomendacoes > 0: detalhamento_pontos['Bônus Recomendações'] = f"+{pts_bonus_recom} pts ({recomendacoes} ocorr.)"
        
        certificacoes = metricas_manuais.get('certificacoes', 0)
        pts_bonus_cert = min(certificacoes, 1) * regras_db.get('bonus_certificacoes', 15) # Limite de 1 certificação
        pts_bonus += pts_bonus_cert
        if certificacoes > 0: detalhamento_pontos['Bônus Certificações'] = f"+{pts_bonus_cert} pts ({certificacoes} ocorr.)"
        ...
        if trein_part > 0: detalhamento_pontos['Bônus Trein. Pacto (Part.)'] = f"+{pts_bonus_tpart} pts ({trein_part} ocorr.)"
        ...
        if trein_aplic > 0: detalhamento_pontos['Bônus Trein. Pacto (Aplic.)'] = f"+{pts_bonus_taplic} pts ({trein_aplic} ocorr.)"
        ...
        if reun_pres > 0: detalhamento_pontos['Bônus Reuniões Presenciais'] = f"+{pts_bonus_reun_pres} pts ({reun_pres} ocorr.)"
        
        pontos += pts_bonus 

        # --- Penalidades (Multiplicadores) ---
        ...
        if reclamacoes > 0: detalhamento_pontos['Penalidade Reclamação'] = f"-{pts_pen_reclam} pts ({reclamacoes} ocorr.)"
        ...
        if perda_prazo > 0: detalhamento_pontos['Penalidade Perda Prazo'] = f"-{pts_pen_prazo} pts ({perda_prazo} ocorr.)"
        ...
        if desc_incomp > 0: detalhamento_pontos['Penalidade Desc. Incomp.'] = f"-{pts_pen_desc} pts ({desc_incomp} ocorr.)"
        ...
        if cancel_resp > 0: detalhamento_pontos['Penalidade Cancelamento Resp.'] = f"-{pts_pen_cancel} pts ({cancel_resp} ocorr.)"
        ...
        if nao_envolv > 0: detalhamento_pontos['Penalidade Não Envolv.'] = f"-{pts_pen_envolv} pts ({nao_envolv} ocorr.)"
        ...
        if nao_preench > 0: detalhamento_pontos['Penalidade Não Preench.'] = f"-{pts_pen_preench} pts ({nao_preench} ocorr.)"
        ...
        if perda_sla > 0: detalhamento_pontos['Penalidade SLA Grupo'] = f"-{pts_pen_sla} pts ({perda_sla} ocorr.)"
        ...
        if final_incomp > 0: detalhamento_pontos['Penalidade Finaliz. Incomp.'] = f"-{pts_pen_final} pts ({final_incomp} ocorr.)"
        ...
        if hora_extra > 0: detalhamento_pontos['Penalidade Hora Extra'] = f"-{pts_pen_he} pts ({hora_extra} ocorr.)"
