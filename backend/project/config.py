...
class Config:
    ...
    if not SECRET_KEY:
        raise ValueError(
            "Nenhuma variável de ambiente SECRET_KEY ou FLASK_SECRET_KEY foi definida."
        )
    ...
    if not R2_CONFIGURADO:
        print(
            "AVISO DE CONFIGURAÇÃO: Uma ou mais variáveis Cloudflare R2 não estão definidas. O upload de imagens está desativado."
        )
    else:
    ...
    if not GOOGLE_OAUTH_ENABLED:
        print(
            "Config: Google OAuth desativado (variáveis ausentes). Define GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI."
        )
    ...
    if EMAIL_DRIVER == "smtp":
        ...
        if not EMAIL_CONFIGURADO:
            print(
                "AVISO: SMTP não configurado (comentários externos não enviarão e-mails)."
            )
    elif EMAIL_DRIVER == "sendgrid":
        EMAIL_CONFIGURADO = all([SENDGRID_API_KEY, SMTP_FROM])
        if not EMAIL_CONFIGURADO:
            print(
                "AVISO: SendGrid não configurado (defina SENDGRID_API_KEY e SMTP_FROM)."
            )
    else:
        EMAIL_CONFIGURADO = False
        print(
            f"AVISO: EMAIL_DRIVER '{EMAIL_DRIVER}' não suportado. Use 'smtp' ou 'sendgrid'."
        )
    ...
    LOG_ROTATION_ENABLED = os.environ.get("LOG_ROTATION_ENABLED", "true").lower() in (
        "1",
        "true",
        "yes",
    )
