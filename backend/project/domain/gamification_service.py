...
def _get_gamification_rules_as_dict():
    ...
    try:
        ...
        if not regras_raw:
            print(
                "AVISO CRÍTICO: Tabela 'gamificacao_regras' está vazia ou não foi encontrada."
            )
            result = {}
        else:
...
def _get_gamification_automatic_data_bulk(
    mes, ano, primeiro_dia_str, fim_ultimo_dia_str, target_cs_email=None
):
    ...
    impl_finalizadas_raw = (
        impl_finalizadas_raw if impl_finalizadas_raw is not None else []
    )
    ...
    for impl in impl_finalizadas_raw:
        ...
        if isinstance(dt_criacao, str):
            try:
                dt_criacao_datetime = datetime.fromisoformat(
                    dt_criacao.replace("Z", "+00:00")
                )
            except ValueError:
                try:
                    if "." in dt_criacao:
                        dt_criacao_datetime = datetime.strptime(
                            dt_criacao, "%Y-%m-%d %H:%M:%S.%f"
                        )
                    else:
                        dt_criacao_datetime = datetime.strptime(
                            dt_criacao, "%Y-%m-%d %H:%M:%S"
                        )
                except ValueError:
                    pass
        ...
        if isinstance(dt_finalizacao, str):
            try:
                dt_finalizacao_datetime = datetime.fromisoformat(
                    dt_finalizacao.replace("Z", "+00:00")
                )
            except ValueError:
                try:
                    if "." in dt_finalizacao:
                        dt_finalizacao_datetime = datetime.strptime(
                            dt_finalizacao, "%Y-%m-%d %H:%M:%S.%f"
                        )
                    else:
                        dt_finalizacao_datetime = datetime.strptime(
                            dt_finalizacao, "%Y-%m-%d %H:%M:%S"
                        )
                except ValueError:
                    pass
        elif isinstance(dt_finalizacao, date) and not isinstance(
            dt_finalizacao, datetime
        ):
            dt_finalizacao_datetime = datetime.combine(
                dt_finalizacao, datetime.min.time()
            )
        ...
        if dt_criacao_datetime and dt_finalizacao_datetime:
            criacao_naive = (
                dt_criacao_datetime.replace(tzinfo=None)
                if dt_criacao_datetime.tzinfo
                else dt_criacao_datetime
            )
            final_naive = (
    ...
    iniciadas_map = {
        row["usuario_cs"]: row["total"]
        for row in impl_iniciadas_raw
    ...
    tarefas_concluidas_raw = (
        tarefas_concluidas_raw if tarefas_concluidas_raw is not None else []
    )
...
def _check_eligibility(
    ...
    media_reunioes_dia_safe = (
        media_reunioes_dia if media_reunioes_dia is not None else 0
    )
    if media_reunioes_dia_safe < min_reunioes_dia:
        elegivel = False
        motivo_inelegibilidade.append(
            f"Média Reuniões/Dia ({media_reunioes_dia_safe:.2f}) < {min_reunioes_dia}"
        )

    if count_finalizadas < min_processos_concluidos:
        elegivel = False
        motivo_inelegibilidade.append(
            f"Impl. Finalizadas ({count_finalizadas}) < {min_processos_concluidos} ({cargo})"
        )
    ...
    if nao_preenchimento >= max_nao_preenchimento + 1:
        elegivel = False
        motivo_inelegibilidade.append(
            f"Não Preenchimento >= {max_nao_preenchimento + 1}"
        )
...
def _calculate_user_gamification_score(
    ...
    if tma_medio is None:
        ...
        tma_medio = (
            round(tma_total_dias / count_finalizadas, 1)
    ...
    if media_reunioes_dia is None:
        ...
        media_reunioes_dia = (
            round(count_reuniao / dias_no_mes, 2) if dias_no_mes > 0 else 0.0
        )
    ...
    if media_acoes_dia is None:
        ...
        media_acoes_dia = (
            round(count_acao_interna / dias_no_mes, 2) if dias_no_mes > 0 else 0.0
        )
...
def _calculate_points(
    ...
    detalhamento_pontos["Assiduidade"] = (
        f"{pts_assiduidade} pts ({assid if assid is not None else 'N/A'}%)"
    )
    ...
    detalhamento_pontos["Média Reuniões/Dia"] = (
        f"{pts_reunioes_dia} pts ({media_reunioes_dia:.2f})"
    )
    ...
    detalhamento_pontos["Média Ações/Dia"] = (
        f"{pts_acoes_dia} pts ({media_acoes_dia:.2f})"
    )
    ...
    pontos += pts_planos
    detalhamento_pontos["Planos Sucesso"] = (
        f"{pts_planos} pts ({psp if psp is not None else 'N/A'}%)"
    )
    ...
    detalhamento_pontos["Impl. Iniciadas"] = (
        f"{pts_iniciadas} pts ({count_iniciadas_final})"
    )
    ...
    detalhamento_pontos["Nota Qualidade"] = (
        f"{pts_qualidade} pts ({nq if nq is not None else 'N/A'}%)"
    )
...
def _calculate_bonus(metricas_manuais, regras_db):
    ...
    if elogios > 0:
        detalhamento_bonus["Bônus Elogios"] = (
            f"+{pts_bonus_elogios} pts ({elogios} ocorr.)"
        )
    ...
    if recomendacoes > 0:
        detalhamento_bonus["Bônus Recomendações"] = (
            f"+{pts_bonus_recom} pts ({recomendacoes} ocorr.)"
        )
    ...
    if certificacoes > 0:
        detalhamento_bonus["Bônus Certificações"] = (
            f"+{pts_bonus_cert} pts ({certificacoes} ocorr.)"
        )
    ...
    if trein_part > 0:
        detalhamento_bonus["Bônus Trein. Pacto (Part.)"] = (
            f"+{pts_bonus_tpart} pts ({trein_part} ocorr.)"
        )
    ...
    if trein_aplic > 0:
        detalhamento_bonus["Bônus Trein. Pacto (Aplic.)"] = (
            f"+{pts_bonus_taplic} pts ({trein_aplic} ocorr.)"
        )
    ...
    if reun_pres > 0:
        detalhamento_bonus["Bônus Reuniões Presenciais"] = (
            f"+{pts_bonus_reun_pres} pts ({reun_pres} ocorr.)"
        )
...
def _calculate_penalties(metricas_manuais, regras_db):
    ...
    if reclamacoes > 0:
        detalhamento_penalidades["Penalidade Reclamação"] = (
            f"-{pts_pen_reclam} pts ({reclamacoes} ocorr.)"
        )
    ...
    if perda_prazo > 0:
        detalhamento_penalidades["Penalidade Perda Prazo"] = (
            f"-{pts_pen_prazo} pts ({perda_prazo} ocorr.)"
        )
    ...
    if desc_incomp > 0:
        detalhamento_penalidades["Penalidade Desc. Incomp."] = (
            f"-{pts_pen_desc} pts ({desc_incomp} ocorr.)"
        )
    ...
    if cancel_resp > 0:
        detalhamento_penalidades["Penalidade Cancelamento Resp."] = (
            f"-{pts_pen_cancel} pts ({cancel_resp} ocorr.)"
        )
    ...
    if nao_envolv > 0:
        detalhamento_penalidades["Penalidade Não Envolv."] = (
            f"-{pts_pen_envolv} pts ({nao_envolv} ocorr.)"
        )
    ...
    if nao_preench > 0:
        detalhamento_penalidades["Penalidade Não Preench."] = (
            f"-{pts_pen_preench} pts ({nao_preench} ocorr.)"
        )
    ...
    if perda_sla > 0:
        detalhamento_penalidades["Penalidade SLA Grupo"] = (
            f"-{pts_pen_sla} pts ({perda_sla} ocorr.)"
        )
    ...
    if final_incomp > 0:
        detalhamento_penalidades["Penalidade Finaliz. Incomp."] = (
            f"-{pts_pen_final} pts ({final_incomp} ocorr.)"
        )
    ...
    if hora_extra > 0:
        detalhamento_penalidades["Penalidade Hora Extra"] = (
            f"-{pts_pen_he} pts ({hora_extra} ocorr.)"
        )
...
def _calculate_user_gamification_score(
    ...
    if elegivel:
        ...
        pts_penalidade, detalhamento_penalidades = _calculate_penalties(
            metricas_manuais, regras_db
        )
    ...
    resultado = {
        "elegivel": elegivel,
        "motivo_inelegibilidade": (
            ", ".join(motivo_inelegibilidade) if motivo_inelegibilidade else None
        ),
    ...
    dados_automaticos_calculados = {
        ...
        "impl_finalizadas_mes": (
            count_finalizadas
            if metricas_manuais.get("impl_finalizadas_mes") is None
            else None
        ),
        "tma_medio_mes": (
            tma_medio if metricas_manuais.get("tma_medio_mes") is None else None
        ),
        "impl_iniciadas_mes": (
            count_iniciadas_final
            if metricas_manuais.get("impl_iniciadas_mes") is None
            else None
        ),
...
def get_gamification_report_data(
    mes, ano, target_cs_email=None, all_cs_users_list=None
):
    """
    ...
    if not regras_db:
        raise ValueError(
            "Falha ao carregar as regras de pontuação da gamificação do banco de dados."
        )
    ...
    usuarios_filtrados = [
        u["usuario"] for u in all_cs_users_list if isinstance(u, dict)
    ]
    ...
    if usuarios_filtrados:
        ...
        metricas_manuais_raw = (
            metricas_manuais_raw if metricas_manuais_raw is not None else []
        )
    ...
    for user_profile in all_cs_users_list:
        ...
        try:
            ...
            resultado, dados_automaticos_calculados = (
            ...
            try:
                ...
                if existing_record_id:
                    ...
                    print(
                        f"AVISO: Métrica manual para {user_email} não tinha ID. Ignorando salvamento automático."
                    )

            except Exception as db_update_err:
                print(
                    f"AVISO: Falha ao salvar resultados calculados da gamificação para {user_email} ({mes}/{ano}): {db_update_err}"
                )
    ...
    report_data_sorted = sorted(
        report_data, key=lambda x: x["pontuacao_final"], reverse=True
    )
