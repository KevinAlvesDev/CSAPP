...
def calculate_time_in_status(impl_id, status_target="parada"):
    ...
    if (
        impl["status"] == status_target
        and status_target == "parada"
        and impl.get("data_finalizacao")
    ):
        ...
        if isinstance(data_inicio_parada_obj, str):
            try:
                data_inicio_parada_datetime = datetime.fromisoformat(
                    data_inicio_parada_obj.replace("Z", "+00:00")
                )
            except ValueError:
                try:
                    if "." in data_inicio_parada_obj:
                        data_inicio_parada_datetime = datetime.strptime(
                            data_inicio_parada_obj, "%Y-%m-%d %H:%M:%S.%f"
                        )
                    else:
                        data_inicio_parada_datetime = datetime.strptime(
                            data_inicio_parada_obj, "%Y-%m-%d %H:%M:%S"
                        )
                except ValueError:
                    print(
                        f"AVISO: Formato de data_finalizacao (str) inválido para impl {impl_id}: {data_inicio_parada_obj}"
                    )
                    return None
        elif isinstance(data_inicio_parada_obj, date) and not isinstance(
            data_inicio_parada_obj, datetime
        ):
            data_inicio_parada_datetime = datetime.combine(
                data_inicio_parada_obj, datetime.min.time()
            )
        ...
        if data_inicio_parada_datetime:
            ...
            try:
                ...
                print(
                    f"AVISO: Erro de tipo ao calcular tempo parado para impl {impl_id}. Verifique timezones. Erro: {te}"
                )
                return None
...
def get_analytics_data(
    ...
    date_field_to_filter = (
        "i.data_finalizacao"
        if target_status in ["finalizada", "cancelada"]
        else "i.data_criacao"
    )
    ...
    end_op, end_date_val = _format_date_for_query(
        end_date, is_end_date=True, is_sqlite=is_sqlite
    )
    if end_op:
        query_impl += (
            f" AND {date_col_expr(date_field_to_filter)} {end_op} {date_param_expr()} "
        )
    ...
    if sort_impl_date in ["asc", "desc"]:
        ...
        query_impl += (
            f" ORDER BY {date_col_expr('i.data_criacao')} {order_dir}, i.nome_empresa "
        )
    else:
    ...
    impl_completas = [
        ...
        if isinstance(impl, dict) and impl.get("tipo") == "completa"
    ]
    ...
    all_cs_profiles = query_db(
        "SELECT usuario, nome, cargo, perfil_acesso FROM perfil_usuario"
    )
    ...
    task_start_date_to_query = (
        ...
        if isinstance(task_start_date, (date, datetime))
        else task_start_date
    ) or default_task_start_date_str
    task_end_date_to_query = (
        task_end_date.strftime("%Y-%m-%d")
        if isinstance(task_end_date, (date, datetime))
        else task_end_date
    ...
    task_start_op, task_start_date_val = _format_date_for_query(
        task_start_date_to_query, is_sqlite=is_sqlite
    )
    if task_start_op:
    ...
    cs_metrics_ranking = {
        p["usuario"]: {
            "email": p["usuario"],
            "nome": p["nome"] or p["usuario"],
            "cargo": p["cargo"] or "N/A",
            "perfil": p["perfil_acesso"] or "Nenhum",
            "impl_total_ranking": 0,
            "tma_sum_ranking": 0,
            "impl_finalizadas_ranking": 0,
            "tma_medio_ranking": "N/A",
        }
        for p in all_cs_profiles
        if p and p.get("usuario")
    }
    ...
    impl_finalizadas_ano_corrente = (
        impl_finalizadas_ano_corrente
        if impl_finalizadas_ano_corrente is not None
        else []
    )
    ...
    for impl in impl_completas:
        ...
        if cs_nome_impl:
            chart_data_ranking_colab[cs_nome_impl] = (
                chart_data_ranking_colab.get(cs_nome_impl, 0) + 1
            )
        ...
        if status == "finalizada":
            ...
            if isinstance(data_inicio_obj, str):
                try:
                    data_inicio_datetime = datetime.fromisoformat(
                        data_inicio_obj.replace("Z", "+00:00")
                    )
                except ValueError:
                    try:
                        if "." in data_inicio_obj:
                            data_inicio_datetime = datetime.strptime(
                                data_inicio_obj, "%Y-%m-%d %H:%M:%S.%f"
                            )
                        else:
                            data_inicio_datetime = datetime.strptime(
                                data_inicio_obj, "%Y-%m-%d %H:%M:%S"
                            )
                    except ValueError:
                        try:
                            data_inicio_datetime = datetime.strptime(
                                data_inicio_obj, "%Y-%m-%d"
                            )
                        except ValueError:
                            pass
            elif isinstance(data_inicio_obj, date) and not isinstance(
                data_inicio_obj, datetime
            ):
                data_inicio_datetime = datetime.combine(
                    data_inicio_obj, datetime.min.time()
                )
            ...
            if data_inicio_datetime:
                agora_naive = agora.replace(tzinfo=None) if agora.tzinfo else agora
                inicio_naive = (
                    data_inicio_datetime.replace(tzinfo=None)
                    if data_inicio_datetime.tzinfo
                    else data_inicio_datetime
                )
                try:
                    dias_passados_delta = agora_naive - inicio_naive
                    dias_passados = (
                        dias_passados_delta.days if dias_passados_delta.days >= 0 else 0
                    )
                except TypeError:
                    dias_passados = -1
    ...
    global_metrics = {
        ...
        "media_tma": (
            round(tma_dias_sum / total_finalizadas, 1)
    ...
    ranking_colab_data = sorted(
        chart_data_ranking_colab.items(), key=lambda item: item[1], reverse=True
    )
...
def get_implants_by_day(start_date=None, end_date=None, cs_email=None):
    ...
    if start_date:
        ...
        if op:
            query += (
                f" AND {date_col_expr('i.data_finalizacao')} {op} {date_param_expr()}"
            )
            args.append(val)
...
def get_cancelamentos_data(cs_email=None, start_date=None, end_date=None):
    ...
    if start_date:
        ...
        if op:
            query += (
                f" AND {date_col_expr('i.data_cancelamento')} {op} {date_param_expr()}"
            )
            args.append(val)
    if end_date:
    ...
    motivo_perc = [
        (c / total_cancel * 100) if total_cancel > 0 else 0 for c in motivo_counts
    ]
    series = {}
    for r in rows:
        dc = _to_dt(r.get("data_cancelamento"))
    ...
    tempos = [
        r["tempo_permanencia_dias"]
        for r in rows
        if r.get("tempo_permanencia_dias") is not None
    ]
    ...
    def pct(p):
        if not tempos_sorted:
            return None
        idx = max(
            0, min(len(tempos_sorted) - 1, int(round(p * (len(tempos_sorted) - 1))))
        )
    ...
    chart_data = {
        ...
        "segmento": {
            "labels": list(seg_counts.keys()),
            "data": list(seg_counts.values()),
        },
        "nivel_receita": {
            "labels": list(nivel_counts.keys()),
            "data": list(nivel_counts.values()),
        },
        "valor_atribuido_buckets": {
            "labels": list(valor_buckets.keys()),
            "data": list(valor_buckets.values()),
        },
        "tempo_dias_hist": {
            "labels": list(tempo_bins.keys()),
            "data": list(tempo_bins.values()),
        },
    }
    metrics = {
