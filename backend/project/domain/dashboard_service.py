...
def get_dashboard_data(
    user_email, filtered_cs_email=None, page=None, per_page=None, use_cache=True
):
    """
    ...
    cache_key = (
        f'dashboard_data_{user_email}_{filtered_cs_email or "all"}_p{page}_pp{per_page}'
    )
    ...
    for impl in impl_list:
        ...
        try:
            if impl.get("tipo") == "modulo" and status in [
                "nova",
                "andamento",
                "atrasada",
                "parada",
        ...
        impl["data_criacao_iso"] = format_date_iso_for_json(
            impl.get("data_criacao"), only_date=True
        )
        impl["data_inicio_efetivo_iso"] = format_date_iso_for_json(
            impl.get("data_inicio_efetivo"), only_date=True
        )
        impl["data_inicio_producao_iso"] = format_date_iso_for_json(
            impl.get("data_inicio_producao"), only_date=True
        )
        ...
        impl["progresso"] = (
            int(round((done_tasks / total_tasks) * 100)) if total_tasks > 0 else 0
        )
        ...
        if data_inicio_obj:
                    ...
                    try:
                        ...
                        try:
                            ...
                            current_app.logger.warning(
                                f"Invalid data_inicio_efetivo format for impl {impl_id}: {data_inicio_obj}"
                            )
            ...
            if data_inicio_datetime:
                try:
                    ...
                    current_app.logger.warning(
                        f"Type error calculating dias_passados for impl {impl_id}: {te}"
                    )
                    dias_passados = -1
        ...
        if status == "finalizada":
            ...
            if data_finalizacao_obj:
                ...
                if isinstance(data_finalizacao_obj, str):
                    try:
                        data_inicio_parada_datetime = datetime.fromisoformat(
                            data_finalizacao_obj.replace("Z", "+00:00")
                        )
                    except ValueError:
                        try:
                            if "." in data_finalizacao_obj:
                                data_inicio_parada_datetime = datetime.strptime(
                                    data_finalizacao_obj, "%Y-%m-%d %H:%M:%S.%f"
                                )
                            else:
                                data_inicio_parada_datetime = datetime.strptime(
                                    data_finalizacao_obj, "%Y-%m-%d %H:%M:%S"
                                )
                        except ValueError:
                            try:
                                data_inicio_parada_datetime = datetime.strptime(
                                    data_finalizacao_obj, "%Y-%m-%d"
                                )
                            except ValueError:
                                current_app.logger.warning(
                                    f"Invalid data_finalizacao format for impl {impl_id}: {data_finalizacao_obj}"
                                )
                elif isinstance(data_finalizacao_obj, date) and not isinstance(
                    data_finalizacao_obj, datetime
                ):
                    data_inicio_parada_datetime = datetime.combine(
                        data_finalizacao_obj, datetime.min.time()
                    )
                ...
                if data_inicio_parada_datetime:
                    try:
                        agora_naive = (
                            agora.replace(tzinfo=None) if agora.tzinfo else agora
                        )
                        parada_naive = (
                        ...
                        dias_parada = (
                            dias_parada_delta.days if dias_parada_delta.days >= 0 else 0
                        )
                    except TypeError as te:
                        current_app.logger.warning(
                            f"Error calculating dias_parada for impl {impl_id}: {te}"
                        )
            ...
            if dias_passados > 25:
                try:
                    execute_db(
                        "UPDATE implantacoes SET status = 'atrasada' WHERE id = %s AND status = 'andamento'",
                        (impl_id,),
                    )
                    status = "atrasada"
            ...
            if dias_passados <= 25:
                try:
                    execute_db(
                        "UPDATE implantacoes SET status = 'andamento' WHERE id = %s AND status = 'atrasada'",
                        (impl_id,),
                    )
                    status = "andamento"
            ...
            current_app.logger.warning(
                f"Unknown or null status for implantacao {impl_id}: '{status}'. Skipping categorization."
            )
        ...
        try:
            rows_affected = execute_db(
                ...
                (
                    ...
                    metrics["implantacoes_atrasadas"],
            ...
            current_app.logger.error(
                f"Failed to update metrics for user {user_email}: {update_err}"
            )
