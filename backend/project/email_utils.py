...
def detect_smtp_settings(user_email):
    ...
    if domain in COMMON:
        ...
        app_logger.info(
            f"Detecção SMTP: {domain} -> {host}:{port} (TLS={use_tls}, SSL={use_ssl})"
        )
        return {
            "host": host,
            "port": port,
    ...
    chosen = candidates[0]
    app_logger.info(
        f"Detecção SMTP: domínio desconhecido '{domain}', usando heurística -> {chosen}:587 (TLS)"
    )
    return {
        "host": chosen,
        "port": 587,
...
def save_smtp_settings(user_email, data):
    ...
    try:
        ...
        if not result:
            app_logger.error(
                f"Persistência falhou ao salvar SMTP para {user_email} (sem retorno do execute_db)"
            )
...
def test_smtp_connection(settings, plain_password):
    ...
    if not _check_password(hashed_password, plain_password):
        ...
        raise smtplib.SMTPAuthenticationError(
            535, b"Senha invalida. Nao corresponde a configuracao salva."
        )
    ...
    app_logger.info(
        f"Testando conexão SMTP para {user} em {host}:{port} (SSL: {use_ssl}, TLS: {use_tls})"
    )
    ...
    try:
        ...
        security_logger.warning(
            f"Falha de autenticacao SMTP (provedor rejeitou) para {user}: {e}"
        )
        raise
        ...
        app_logger.warning(
            f"Falha de conectividade SMTP para {user} em {host}:{port}: {e}"
        )
...
def send_email_global(
    subject, body_html, recipients, from_name=None, reply_to=None, body_text=None
):
    """
    ...
    if not from_addr:
        raise ValueError(
            "Configuração de e-mail inválida: remetente (SMTP_FROM) ausente."
        )
    ...
    if driver == "sendgrid":
        ...
        try:
            disable_tracking_env = str(
                cfg.get("SENDGRID_DISABLE_TRACKING", "")
            ).lower() in ("1", "true", "yes")
            domains_cfg = cfg.get("EMAIL_DISABLE_TRACKING_DOMAINS")
            disable_for_domain = False
            if domains_cfg:
                domains_set = {
                    d.strip().lower() for d in str(domains_cfg).split(",") if d.strip()
                }
                for r in recipients:
                    if "@" in r:
        ...
        try:
            ...
            if resp.status_code == 202:
                msg_id = resp.headers.get("X-Message-Id") or resp.headers.get(
                    "X-Message-ID"
                )
                if msg_id:
                    app_logger.info(
                        f"E-mail global (SendGrid) enviado para {recipients} (Message-ID: {msg_id})"
                    )
                else:
                    app_logger.info(
                        f"E-mail global (SendGrid) enviado para {recipients}"
                    )
                return True
            else:
