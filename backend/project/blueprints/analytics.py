...
def analytics_dashboard():
    ...
    try:
        ...
        current_task_start_date = task_start_date or analytics_data.get(
            "default_task_start_date"
        )
        current_task_end_date = task_end_date or analytics_data.get(
            "default_task_end_date"
        )

        return render_template(
            "analytics.html",
            kpi_cards=analytics_data.get("kpi_cards", {}),
            implantacoes_lista_detalhada=analytics_data.get(
                "implantacoes_lista_detalhada", []
            ),
            modules_implantacao_lista=analytics_data.get(
                "modules_implantacao_lista", []
            ),
            chart_data=analytics_data.get("chart_data", {}),
            implantacoes_paradas_lista=analytics_data.get(
                "implantacoes_paradas_lista", []
            ),
            implantacoes_canceladas_lista=analytics_data.get(
                "implantacoes_canceladas_lista", []
            ),
...
def api_implants_by_day():
    ...
    try:
        ...
        payload = get_implants_by_day(
            start_date=start_date, end_date=end_date, cs_email=cs_email
        )
...
def cancelamentos_dashboard():
    ...
    try:
        ...
        payload = get_cancelamentos_data(
            cs_email=cs_email, start_date=start_date, end_date=end_date
        )
        return render_template(
            "cancelamentos.html",
            **payload,
...
def api_gamification_rank():
    ...
    try:
        ...
        return (
            jsonify(
                {"ok": False, "error": "Parâmetros month/year devem ser inteiros."}
            ),
            400,
        )
    except Exception as e:
