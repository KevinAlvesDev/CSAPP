...
def profile_modal():
    ...
    return render_template(
        "modals/_perfil_content.html", r2_configurado=g.R2_CONFIGURED
    )
...
def save_profile():
    ...
    if foto and g.R2_CONFIGURED and r2_client:
        try:
            ...
            app_logger.info(
                f"Foto de perfil carregada para {g.user_email} em {foto_url}"
            )
            ...
            try:
                ...
                if foto_bytes is None:
                    raise ValueError(
                        "Falha ao obter conteúdo da foto para salvar localmente"
                    )
                with open(save_path, "wb") as f_out:
                    f_out.write(foto_bytes)
                foto_url = f"/static/uploads/profile/{unique_name}"
                app_logger.info(
                    f"Foto de perfil salva localmente para {g.user_email} em {foto_url}"
                )
            except Exception as e2:
                app_logger.error(
                    f"Falha ao salvar foto local para {g.user_email}: {e2}"
                )
    elif foto:
        try:
            try:
