import os
import time
from flask import Blueprint, request, flash, redirect, url_for, g, current_app
...
from ..task_definitions import MODULO_PENDENCIAS
...
def criar_implantacao():
    ...
    try:
        ...
        nome_empresa = sanitize_string(
            request.form.get("nome_empresa", ""), max_length=200
        )
        if not nome_empresa:
            raise ValidationError("Nome da empresa é obrigatório.")

        usuario_atribuido = sanitize_string(
            request.form.get("usuario_atribuido_cs", ""), max_length=100
        )
    ...
    try:
        ...
        if existente:
            ...
            flash(
                f'Já existe uma implantação ativa para "{nome_empresa}" (status: {status_existente}). Evite duplicados.',
                "error",
            )
            return redirect(url_for("main.dashboard"))
...
def criar_implantacao_modulo():
    ...
    try:
        ...
        nome_empresa = sanitize_string(
            request.form.get("nome_empresa_modulo", ""), max_length=200
        )
        if not nome_empresa:
            raise ValidationError("Nome da empresa é obrigatório.")

        usuario_atribuido = sanitize_string(
            request.form.get("usuario_atribuido_cs_modulo", ""), max_length=100
        )
        if not usuario_atribuido:
            raise ValidationError("Implantador atribuído é obrigatório.")

        modulo_tipo = sanitize_string(
            request.form.get("modulo_tipo", ""), max_length=50
        )
        modulo_opcoes = {
    ...
    try:
        ...
        print(
            f"Falha ao verificar duplicidade de empresa (módulo) '{nome_empresa}': {e}"
        )
        flash("Erro ao validar duplicidade de empresa.", "error")
        return redirect(url_for("main.dashboard"))
...
def iniciar_implantacao():
    ...
    try:
        ...
        implantacao_id = validate_integer(
            request.form.get("implantacao_id"), min_value=1
        )
    except ValidationError as e:
    ...
    try:
        ...
        if impl.get("status") not in ["nova", "futura", "sem_previsao"]:
            flash(
                f'Operação negada. Implantação com status "{impl.get("status")}" não pode ser iniciada.',
                "error",
            )
            return redirect(request.referrer or dest_url_fallback)
...
def agendar_implantacao():
    ...
    try:
        ...
        flash(
            "Data de início inválida. Formatos aceitos: DD/MM/AAAA, MM/DD/AAAA, AAAA-MM-DD.",
            "error",
        )
        return redirect(url_for("main.dashboard"))
    ...
    try:
        ...
        flash(
            f'Implantação "{impl.get("nome_empresa")}" movida para "Futuras" com início em {data_prevista}.',
            "success",
        )
        try:
            clear_user_cache(usuario_cs_email)
...
def marcar_sem_previsao():
    ...
    try:
        ...
        if impl.get("status") != "nova":
            flash(
                'Apenas implantações "Novas" podem ser marcadas como Sem previsão.',
                "warning",
            )
            return redirect(url_for("main.dashboard"))
        ...
        ver = query_db(
            "SELECT status FROM implantacoes WHERE id = %s", (implantacao_id,), one=True
        )
...
def finalizar_implantacao():
    ...
    try:
        app_logger.info(
            f"Finalizar solicitada por {usuario_cs_email} para implantacao_id={implantacao_id}"
        )
        impl = query_db(
            "SELECT usuario_cs, nome_empresa, status FROM implantacoes WHERE id = %s",
            (implantacao_id,),
        ...
        if not impl:
            app_logger.warning(
                f"Finalizar negada: implantação inexistente id={implantacao_id} user={usuario_cs_email}"
            )
            flash("Implantação não encontrada.", "error")
            return redirect(dest_url)
        if impl.get("usuario_cs") != usuario_cs_email:
            app_logger.warning(
                f"Finalizar negada: permissão user={usuario_cs_email} não é dono da implantação id={implantacao_id}"
            )
            flash("Permissão negada. Esta implantação não pertence a você.", "error")
            return redirect(dest_url)
        if impl.get("status") != "andamento":
            app_logger.warning(
                f"Finalizar negada: status atual='{impl.get('status')}' id={implantacao_id} user={usuario_cs_email}"
            )
            flash(
        ...
        app_logger.info(
            f"Finalizar validação tarefas: total={total_all} pendentes={pending_tasks.get('total', 0) if pending_tasks else 0} id={implantacao_id} user={usuario_cs_email}"
        )
        if total_all == 0:
        ...
        if pending_tasks and pending_tasks.get("total", 0) > 0:
            ...
            flash(
                f"Não é possível finalizar: {total_pendentes} tarefa(s) pendente(s). Pendentes: {nomes_txt}...",
                "error",
            )
            return redirect(dest_url)
        ...
        try:
            ...
            flash(
                "Data da finalização inválida. Formatos aceitos: DD/MM/AAAA, MM/DD/AAAA, AAAA-MM-DD.",
                "error",
            )
            return redirect(dest_url)
        ...
        logar_timeline(
            ...
            usuario_cs_email,
            "status_alterado",
            f'Implantação "{impl.get("nome_empresa", "N/A")}" finalizada manually.',
        )
        ...
        try:
            ...
            pass
        app_logger.info(
            f"Finalizar concluída id={implantacao_id} user={usuario_cs_email}"
        )
        try:
            clear_user_cache(usuario_cs_email)
            clear_implantacao_cache(implantacao_id)
        ...
        app_logger.error(
            f"Erro ao finalizar implantação id={implantacao_id} user={usuario_cs_email}: {e}"
        )
...
def parar_implantacao():
    ...
    try:
        ...
        flash(
            "Data da parada inválida. Formatos aceitos: DD/MM/AAAA, MM/DD/AAAA, AAAA-MM-DD.",
            "error",
        )
        return redirect(dest_url)
    ...
    try:
        ...
        if (
            not impl
            or impl.get("usuario_cs") != usuario_cs_email
            or impl.get("status") != "andamento"
        ):
...
def reabrir_implantacao():
    ...
    try:
        ...
        flash(
            'Implantação reaberta com sucesso e movida para "Em Andamento".', "success"
        )
        try:
            clear_user_cache(usuario_cs_email)
            clear_implantacao_cache(implantacao_id)
...
def atualizar_detalhes_empresa():
    ...
    try:
        ...
        campos = {
            ...
            "data_inicio_producao": normalize_date_str(
                get_form_value("data_inicio_producao")
            ),
            "data_final_implantacao": normalize_date_str(
                get_form_value("data_final_implantacao")
            ),
            "data_inicio_efetivo": normalize_date_str(
                get_form_value("data_inicio_efetivo")
            ),
        ...
        logar_timeline(
            ...
            usuario_cs_email,
            "detalhes_alterados",
            "Detalhes da empresa/cliente foram atualizados.",
        )
...
def transferir_implantacao():
    ...
    try:
        impl = query_db(
            "SELECT nome_empresa, usuario_cs FROM implantacoes WHERE id = %s",
            (implantacao_id,),
            one=True,
        )
        if not impl:
        ...
        execute_db(
            "UPDATE implantacoes SET usuario_cs = %s WHERE id = %s",
            (novo_usuario_cs, implantacao_id),
        )
        logar_timeline(
            implantacao_id,
            usuario_cs_email,
...
def excluir_implantacao():
    ...
    try:
        comentarios_img = query_db(
            """ SELECT c.imagem_url FROM comentarios c JOIN tarefas t ON c.tarefa_id = t.id WHERE t.implantacao_id = %s AND c.imagem_url IS NOT NULL AND c.imagem_url != '' """,
            (implantacao_id,),
        )
            ...
            for c in comentarios_img:
                ...
                if imagem_url and imagem_url.startswith(public_url_base):
                    try:
                        ...
                        print(
                            f"Aviso: Falha ao excluir R2 (key: {object_key}). Erro: {e_delete.response['Error']['Code']}"
                        )
                    except Exception as e_delete:
                        print(
                            f"Aviso: Falha ao excluir R2 (key: {object_key}). Erro: {e_delete}"
                        )
        else:
            print(
                "Aviso: R2 não configurado ou variáveis ausentes; exclusão seguirá apenas no banco de dados."
            )
        execute_db("DELETE FROM implantacoes WHERE id = %s", (implantacao_id,))
        flash(
            "Implantação e todos os dados associados foram excluídos com sucesso.",
            "success",
        )
        try:
            clear_user_cache(usuario_cs_email)
...
def adicionar_tarefa():
    ...
    anchor = (
        "pendencias-content"
        if tarefa_pai == MODULO_PENDENCIAS
        else "checklist-treinamentos-content"
    )
    ...
    try:
        ...
        if impl.get("status") == "finalizada":
            flash(
                "Não é possível adicionar tarefas a implantações finalizadas.",
                "warning",
            )
            return redirect(dest_url)
        max_ordem = query_db(
            "SELECT MAX(ordem) as max_o FROM tarefas WHERE implantacao_id = %s AND tarefa_pai = %s",
            (implantacao_id, tarefa_pai),
            one=True,
        )
        ...
        logar_timeline(
            ...
            usuario_cs_email,
            "tarefa_adicionada",
            f"Tarefa '{tarefa_filho}' adicionada ao módulo '{tarefa_pai}'.",
        )
...
def cancelar_implantacao():
    ...
    if not all([implantacao_id, data_cancelamento, motivo]):
        flash(
            "Todos os campos (Data, Motivo Resumo) são obrigatórios para cancelar.",
            "error",
        )
        return redirect(url_for("main.ver_implantacao", impl_id=implantacao_id))
    ...
    try:
        ...
        flash(
            "Data do cancelamento inválida. Formatos aceitos: DD/MM/AAAA, MM/DD/AAAA, AAAA-MM-DD.",
            "error",
        )
        return redirect(url_for("main.ver_implantacao", impl_id=implantacao_id))
    ...
    try:
        ...
        if file and utils.allowed_file(file.filename):
            ...
            if r2_ok:
                ...
                comprovante_url = url_for(
                    "main.serve_upload",
                    filename=f"comprovantes_cancelamento/{nome_unico}",
                )
        else:
