...
def manage_users():
    ...
    try:
        ...
        perfis_disponiveis = current_app.config.get("PERFIS_DE_ACESSO") or [
            "Visitante",
            "Implantador",
            "Gestor",
            "Administrador",
        ...
        return render_template(
            "manage_users.html", users=users_data, perfis_list=perfis_disponiveis
        )
...
def perform_backup():
    ...
    with db_connection() as (conn, db_type):
        if db_type == "sqlite":
            ...
            db_filename = (
                "dashboard_simples_test.db" if is_testing else "dashboard_simples.db"
            )
            ...
            return {
                "type": "sqlite",
                "backup_file": target.replace(base_dir + os.sep, ""),
            }
            ...
            return {
                "type": "postgres",
                "backup_file": zip_path.replace(base_dir + os.sep, ""),
            }
        else:
...
def update_user_profile():
    ...
    if usuario_alvo == g.user_email:
        security_logger.warning(
            f"Admin {g.user_email} tentou alterar o próprio perfil via 'update_user_profile'"
        )
        return (
            jsonify(
                {
                    "ok": False,
                    "error": "Não pode alterar o seu próprio perfil por esta interface.",
                }
            ),
            403,
        )
    ...
    try:
        ...
        user_exists = query_db(
            "SELECT 1 FROM perfil_usuario WHERE usuario = %s", (usuario_alvo,), one=True
        )
        if not user_exists:
        ...
        management_logger.info(
            f"Admin {g.user_email} atualizou o perfil de {usuario_alvo} para {novo_perfil}"
        )
        return jsonify({"ok": True})

    except Exception as e:
        management_logger.error(
            f"Erro ao atualizar perfil de {usuario_alvo} por {g.user_email}: {e}"
        )
...
def update_user_perfil():
    ...
    if usuario_alvo == g.user_email:
        security_logger.warning(
            f"Admin {g.user_email} tentou alterar o próprio perfil via 'update_user_perfil'"
        )
        flash(
            "Você não pode alterar o seu próprio perfil por esta interface.", "warning"
        )
        if request.headers.get("HX-Request") == "true":
            users_data = (
                query_db(
    ...
    if usuario_alvo == ADMIN_EMAIL and (
        novo_perfil is None or novo_perfil != PERFIL_ADMIN
    ):
        security_logger.warning(
            "Tentativa de rebaixar administrador detectada por " + str(g.user_email)
        )
    ...
    try:
        ...
        try:
            ...
            target_is_admin = (
                target_is_admin and target_is_admin.get("perfil_acesso") == PERFIL_ADMIN
            )
        except Exception:
        ...
        if target_is_admin and (g.perfil or {}).get("perfil_acesso") != PERFIL_ADMIN:
            ...
            flash(
                "Apenas Administradores podem alterar perfis de Administradores.",
                "warning",
            )
            if request.headers.get("HX-Request") == "true":
                users_data = (
...
def delete_user():
    ...
    if usuario_alvo == g.user_email:
        security_logger.warning(
            f"Tentativa de exclusão do próprio usuário por {g.user_email}"
        )
    ...
    if usuario_alvo == ADMIN_EMAIL:
        security_logger.warning(
            "Tentativa de exclusão do administrador principal detectada por "
            + str(g.user_email)
        )
    ...
    try:
        ...
        perfil = query_db(
            "SELECT foto_url FROM perfil_usuario WHERE usuario = %s",
            (usuario_alvo,),
            one=True,
        )
        ...
        implantacoes_ids = (
            query_db(
                "SELECT id FROM implantacoes WHERE usuario_cs = %s", (usuario_alvo,)
            )
            or []
        )
        ...
        management_logger.info(
            f"Usuário {usuario_alvo} excluído por {g.user_email} (implantações vinculadas removidas: {len(implantacoes_ids)})"
        )
        flash(
            f"Usuário e {len(implantacoes_ids)} implantações vinculadas excluídos.",
            "success",
        )
        if request.headers.get("HX-Request") == "true":
            users_data = (
        ...
        management_logger.error(
            f"Erro ao excluir usuário {usuario_alvo} por {g.user_email}: {e}"
        )
        flash(f"Erro de banco de dados: {e}", "error")
        if request.headers.get("HX-Request") == "true":
            users_data = (
...
def cleanup_orphan_implantacoes():
    ...
    try:
        ...
        count = stats.get("c", 0)
        management_logger.info(
            f"Admin {g.user_email} removeu {count} implantações órfãs"
        )
        flash(f"{count} implantações órfãs removidas.", "success")
        return redirect(url_for("management.manage_users"))
    except Exception as e:
        management_logger.error(
            f"Erro ao remover implantações órfãs por {g.user_email}: {e}"
        )
