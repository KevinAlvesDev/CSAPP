...
def save_gamification_rules_from_modal():
    ...
    try:
        ...
        for key, value in request.form.items():
            if key.startswith("regra-"):
                ...
                try:
                    ...
                    valor_pontos = validate_integer(
                        value, min_value=-1000, max_value=10000
                    )
                    updates_to_make.append((valor_pontos, regra_id))
                except ValidationError as e:
                    print(
                        f"AVISO: Valor inválido recebido para {regra_id}: {value} - Erro: {str(e)}"
                    )
        ...
        flash(
            f"{total_atualizado} regras de pontuação foram atualizadas com sucesso!",
            "success",
        )
...
def manage_gamification_metrics():
    ...
    try:
        target_mes = validate_integer(
            request.values.get("mes", default_mes), min_value=1, max_value=12
        )
        target_ano = validate_integer(
            request.values.get("ano", default_ano), min_value=2020, max_value=max_ano
        )
    ...
    if target_cs_email:
        ...
        try:
            ...
            tma_data_map, iniciadas_map, tarefas_map = (
                _get_gamification_automatic_data_bulk(
                    target_mes,
                    target_ano,
                    primeiro_dia_str,
            ...
            print(
                f"Erro ao calcular métricas automáticas para {target_cs_email}: {e_auto}"
            )
    ...
    if request.method == "POST":
        if not target_cs_email:
            flash(
                "É necessário selecionar um usuário para salvar as métricas.", "error"
            )
        ...
        try:
            ...
            for p in percentuais:
                ...
                if v is not None and (v < 0 or v > 100):
                    flash(
                        f'Valor inválido para {p.replace("_", " ")}: {v}. Deve estar entre 0 e 100.',
                        "error",
                    )
                    return redirect(
                        url_for(
            ...
            try:
                impl_finalizadas_auto = int(
                    metricas_automaticas.get("impl_finalizadas_mes", 0) or 0
                )
            ...
            try:
                impl_iniciadas_auto = int(
                    metricas_automaticas.get("impl_iniciadas_mes", 0) or 0
                )
            ...
            try:
                reunioes_media_auto = float(
                    metricas_automaticas.get("reunioes_concluidas_dia_media", 0) or 0
                )
                acoes_media_auto = float(
                    metricas_automaticas.get("acoes_concluidas_dia_media", 0) or 0
                )
            ...
            ocorrencias_fields = [
                f
                for f in int_fields
                if f not in ["impl_finalizadas_mes", "impl_iniciadas_mes"]
            ]
            for f in ocorrencias_fields:
                ...
                if v is not None:
                    if v < 0:
                        flash(
                            f'O valor para {f.replace("_", " ")} não pode ser negativo.',
                            "error",
                        )
                        return redirect(
                            url_for(
                    ...
                    if v > 1000:
                        flash(
                            f'Valor muito alto para {f.replace("_", " ")}: {v}. Verifique e tente novamente.',
                            "error",
                        )
                        return redirect(
                            url_for(
            ...
            if existing_record_id:
                set_clauses = [
                    ...
                    if key not in ["usuario_cs", "mes", "ano"]
                ]
                sql_update = f"""
...
def gamification_report():
    ...
    try:
        ...
        selected_year = validate_integer(
            default_ano, min_value=2020, max_value=hoje.year + 10
        )
    except ValidationError:
