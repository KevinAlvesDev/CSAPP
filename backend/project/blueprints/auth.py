...
def _sync_user_profile(user_email, user_name, auth0_user_id):
    ...
    try:
        ...
        usuario_existente = query_db(
            "SELECT usuario FROM usuarios WHERE usuario = %s", (user_email,), one=True
        )

        perfil_existente = query_db(
        ...
        if not usuario_existente:
            ...
            try:
                senha_placeholder = generate_password_hash(
                    auth0_user_id + current_app.secret_key
                )
                execute_db(
                ...
                print(
                    f"AVISO: Tentativa de inserir usuário duplicado {user_email}: {e}"
                )
        ...
        if not perfil_existente:
            ...
            auth_logger.info(
                f"User profile created: {user_email} with role {perfil_acesso_final}"
            )
            ...
            if perfil_acesso_atual.get("perfil_acesso") != PERFIL_ADMIN:
                ...
                print(
                    f"Perfil de acesso atualizado: {user_email} forçado para {PERFIL_ADMIN}."
                )
        ...
        auth_logger.error(
            f"Critical error syncing user profile {user_email}: {str(db_error)}"
        )
    ...
    def decorated_function(*args, **kwargs):
        ...
        if g.perfil.get("perfil_acesso") is None:
            try:
                ...
                _sync_user_profile(
                    g.user_email, g.user.get("name", g.user_email), g.user.get("sub")
                )

                g.perfil = query_db(
                    "SELECT * FROM perfil_usuario WHERE usuario = %s",
                    (g.user_email,),
                    one=True,
                )
                ...
                print(
                    f"Erro no _sync_user_profile durante o fallback do @login_required: {e}"
                )

        perfil_acesso_debug = (
            g.perfil.get("perfil_acesso") if g.perfil else "NÃO CARREGADO"
        )
        auth_logger.info(
            f"User authenticated: {g.user_email}, Role: {perfil_acesso_debug}, Path: {request.path}"
        )
        print(
...
def forgot_password():
    ...
    try:
        ...
        flash(
            "Sistema de e-mail não está configurado. Use o link para redefinir:",
            "warning",
        )
...
def login():
    ...
    usuario = query_db(
        "SELECT usuario, senha FROM usuarios WHERE usuario = %s", (email,), one=True
    )
    ...
    perfil = query_db(
        "SELECT nome FROM perfil_usuario WHERE usuario = %s", (email,), one=True
    )
    nome = perfil.get("nome") if perfil else email

    session["user"] = {"email": email, "name": nome, "sub": f"local|{email}"}
...
def callback():
    ...
    try:
        ...
        flash(
            f"Erro durante a autenticação: Algo deu errado, por favor tente novamente.",
            "error",
        )
        session.clear()
...
def dev_login():
    ...
    if flask_env == "production" or (not flask_debug and not use_sqlite):
        security_logger.warning(
            f"Tentativa de acesso a /dev-login em ambiente de produção"
        )
        abort(404)
...
def dev_login_as():
    ...
    if flask_env == "production" or (not flask_debug and not use_sqlite):
        security_logger.warning(
            f"Tentativa de acesso a /dev-login-as em ambiente de produção"
        )
        abort(404)
    ...
    try:
        ...
        if email != ADMIN_EMAIL:
            try:
                ...
                auth_logger.warning(
                    f"Não foi possível definir perfil Implantador para {email}: {role_err}"
                )
    except Exception as e:
...
def register():
    ...
    if usuario_existente:
        flash(
            "Este e-mail já está cadastrado. Faça login ou use outro e-mail.", "error"
        )
        return render_template("register.html", auth0_enabled=False)
    ...
    try:
        ...
        execute_db(
            "INSERT INTO usuarios (usuario, senha) VALUES (%s, %s)", (email, senha_hash)
        )
