...
def home():
    ...
    return render_template(
        "login.html", auth0_enabled=auth0_enabled, use_custom_auth=not auth0_enabled
    )
...
def dashboard():
    ...
    try:
        ...
        dashboard_data, metrics = get_dashboard_data(
            user_email, filtered_cs_email=current_cs_filter, use_cache=False
        )
        ...
        return render_template(
            ...
            all_cs_users=[],
            PERFIS_COM_CRIACAO=PERFIS_COM_CRIACAO,
            is_manager=is_manager_erro,
            current_cs_filter=current_cs_filter_erro,
        )
...
def ver_implantacao(impl_id):
    ...
    try:
        ...
        context_data = get_implantacao_details(
            impl_id=impl_id, usuario_cs_email=g.user_email, user_perfil=g.perfil
        )

        return render_template("implantacao_detalhes.html", **context_data)

    except ValueError as e:

        from ..logging_config import get_logger
        get_logger("implantacao_details").error(
            f"Falha de acesso aos detalhes da implantação ID {impl_id}: {e}", exc_info=True
        )
        flash(str(e), "error")
        return redirect(url_for("main.dashboard"))
        ...
        logger = get_logger("implantacao_details")
        logger.error(
            f"Erro ao carregar detalhes da implantação ID {impl_id}: {e}", exc_info=True
        )
