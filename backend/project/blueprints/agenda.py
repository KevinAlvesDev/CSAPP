...
def agenda_home():
    ...
    try:
        agenda_logger.debug(
            f"Token na sessão: {bool(token)}; chaves={list(token.keys()) if token else None}"
        )
    except Exception:
        pass
    if not token:
    ...
    try:
        ...
        agenda_logger.error(
            f"Erro ao buscar eventos do Google Calendar para {g.user_email}: {e}",
            exc_info=True,
        )
        try:
        ...
        return render_template(
            "agenda.html", events=[], google_connected=False, view=view
        )
...
def agenda_create_event():
    ...
    if all_day:
        ...
        if not (start_time and end_time):
            return (
                jsonify({"ok": False, "error": "startTime e endTime são obrigatórios"}),
                400,
            )
    ...
    conference = bool(
        (payload.get("conference") is True) or (payload.get("createMeetLink") is True)
    )
    extra_params = None
    if conference:
        ...
        if resp.status_code == 401:
            return (
                jsonify(
                    {
                        "ok": False,
                        "error": "Token expirado. Refaça a conexão com o Google.",
                    }
                ),
                401,
            )
...
def agenda_update_event(event_id):
    ...
    payload = request.get_json(silent=True) or {}
    calendar_id = (
        payload.get("calendarId") or request.args.get("calendarId") or "primary"
    )
    ...
    if all_day is True:
        if not date_str:
            return (
                jsonify({"ok": False, "error": "date é obrigatório para dia inteiro"}),
                400,
            )
        event_body["start"] = {"date": date_str}
        event_body["end"] = {"date": date_str}
        ...
        if not (date_str and start_time and end_time):
            return (
                jsonify(
                    {"ok": False, "error": "date, startTime e endTime são obrigatórios"}
                ),
                400,
            )
        start_dt = f"{date_str}T{start_time}:00"
