...
def get_implantacao(impl_id):
    ...
    try:
        ...
        return jsonify({"ok": True, "data": {"implantacao": impl, "tarefas": tarefas}})
...
def marcar_atrasada(impl_id):
    try:
        impl = query_db(
            "SELECT status, data_inicio_efetivo, nome_empresa FROM implantacoes WHERE id = %s",
            (impl_id,),
            one=True,
        )
        if not impl:
        ...
        if not di_dt:
            return (
                jsonify({"ok": False, "error": "Data de inicio efetivo ausente"}),
                400,
            )
        agora = datetime.now()
        ...
        if dias > 25 and status == "andamento":
            execute_db(
                "UPDATE implantacoes SET status = 'atrasada' WHERE id = %s", (impl_id,)
            )
        ...
        if dias <= 25 and status == "atrasada":
            execute_db(
                "UPDATE implantacoes SET status = 'andamento' WHERE id = %s", (impl_id,)
            )
