...
def setup_logging(app):
    ...
    if rotation_enabled:
        ...
        implantacao_error_handler = TimedRotatingFileHandler(
            os.path.join(log_dir, "implantacao_details_errors.log"),
            when="midnight",
            backupCount=retention_days,
            encoding="utf-8",
        )
        implantacao_error_handler.setLevel(logging.ERROR)
    else:
        ...
        error_handler.setLevel(logging.ERROR)
        implantacao_error_handler = logging.FileHandler(
            os.path.join(log_dir, "implantacao_details_errors.log"), encoding="utf-8"
        )
        implantacao_error_handler.setLevel(logging.ERROR)
    ...
    implantacao_error_handler.setFormatter(formatter)
    implantacao_error_handler.addFilter(ContextFilter())
    ...
    for name in [
        ...
        if name == "implantacao_details":
            named_logger.addHandler(implantacao_error_handler)
