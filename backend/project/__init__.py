...
def create_app():
    ...
    try:
        ...
        load_local_env = os.environ.get("CSAPP_LOAD_DOTENV", "1").lower() in (
            "1",
            "true",
            "yes",
        )
        if load_local_env and not is_prod:
    ...
    try:
        if app.config.get("USE_SQLITE_LOCALLY", False) or app.config.get(
            "DEBUG", False
        ):
    ...
    try:
        ...
        print(
            f"[Startup] Google Config: CLIENT_ID={'definido' if gi else 'vazio'}, CLIENT_SECRET={'definido' if gs else 'vazio'}, REDIRECT_URI={'definido' if gr else 'vazio'}"
        )
        print(
            f"[Startup] GOOGLE_OAUTH_ENABLED={bool(app.config.get('GOOGLE_OAUTH_ENABLED'))}"
        )
    except Exception as e:
                ...
                try:
                    ...
                    if not admin_exists:
                        ...
                        execute_db(
                            ...
                            (seeded_hash, ADMIN_EMAIL),
                        )
                    perfil_exists = query_db(
                        "SELECT usuario FROM perfil_usuario WHERE usuario = %s",
                        (ADMIN_EMAIL,),
                        one=True,
                    )
    ...
    try:
        if app.config.get("GOOGLE_OAUTH_ENABLED", False):
            ...
            gr = app.config.get("GOOGLE_REDIRECT_URI")
            print(
                f"[Startup] Pós-registro OAuth: CLIENT_ID={'definido' if gi else 'vazio'}, CLIENT_SECRET={'definido' if gs else 'vazio'}, REDIRECT_URI={'definido' if gr else 'vazio'}"
            )
        else:
    ...
    try:
        ...
        print(
            f"ERRO CRÍTICO: Falha ao carregar regras de gamificação na inicialização: {e}"
        )
    ...
    def load_logged_in_user():
        ...
        if g.user_email:
            try:
                ...
                print(
                    f"ALERTA: Falha ao buscar perfil no before_request para {g.user_email}: {e}"
                )

                if (
                    not current_app.config.get("AUTH0_ENABLED", True)
                    and g.user_email == ADMIN_EMAIL
                ):
                    g.perfil = {
        ...
        try:
            ...
            print(
                f"ALERTA: Falha ao carregar regras de gamificação no before_request (a partir do app.context): {e}"
            )
    ...
    def internal_server_error(e):
        ...
        flash(
            "Ocorreu um erro interno no servidor. Redirecionando para o Dashboard.",
            "error",
        )
        return redirect(url_for("main.dashboard"))
    ...
    def set_security_headers(response):
        ...
        is_prod = not current_app.config.get(
            "DEBUG", False
        ) and not current_app.config.get("USE_SQLITE_LOCALLY", False)
        script_src = (
