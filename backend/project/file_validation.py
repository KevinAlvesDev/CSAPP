...
def validate_file_content(file_stream, filename):
    """
    Valida o conteúdo do arquivo usando python-magic.
...
def validate_file_size(file_stream, max_size=MAX_FILE_SIZE):
    ...
    try:
        ...
        if file_size > max_size:
            ...
            return (
                False,
                f"Arquivo muito grande ({file_size_mb:.2f} MB). Máximo: {max_size_mb:.0f} MB",
                file_size,
            )
...
def validate_uploaded_file(file, filename=None):
    """
    Validação completa de arquivo enviado.
    Verifica: nome, extensão, tamanho e conteúdo (MIME type).

    Retorna: (is_valid: bool, error_message: str, metadata: dict)
    """
    if not file:
    ...
    content_valid, content_error, mime_type = validate_file_content(
