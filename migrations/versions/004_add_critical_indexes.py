...
def upgrade():
    ...
    def safe_create_index(index_name, table_name, columns, desc=False):
        try:
            if desc and isinstance(columns, list) and len(columns) == 1:
                ...
                op.execute(
                    f"CREATE INDEX {index_name} ON {table_name}({columns[0]} DESC)"
                )
            else:
    ...
    safe_create_index(
        "idx_implantacoes_data_criacao", "implantacoes", ["data_criacao"], desc=True
    )
    safe_create_index(
        "idx_implantacoes_usuario_status", "implantacoes", ["usuario_cs", "status"]
    )
    ...
    safe_create_index(
        "idx_tarefas_data_conclusao", "tarefas", ["data_conclusao"], desc=True
    )
    safe_create_index(
        "idx_tarefas_impl_concluida", "tarefas", ["implantacao_id", "concluida"]
    )
    safe_create_index("idx_tarefas_tag", "tarefas", ["tag"])

    safe_create_index("idx_comentarios_tarefa_id", "comentarios", ["tarefa_id"])
    safe_create_index(
        "idx_comentarios_data_criacao", "comentarios", ["data_criacao"], desc=True
    )
    ...
    safe_create_index(
        "idx_timeline_data_evento", "timeline", ["data_evento"], desc=True
    )

    safe_create_index(
        "idx_gamificacao_usuario_mes",
