...
@pytest.mark.parametrize("new_tag", ["Cliente", "Reunião", "Ação interna"])
def test_update_tag_logs_objective(new_tag):
    app = create_app_for_tests()
    client = app.test_client()
    impl_id, item_id = setup_implantacao_and_item(app)
    res = client.patch(f"/api/checklist/item/{item_id}/tag", json={"tag": new_tag})
    assert res.status_code == 200
    assert res.get_json().get('tag') == new_tag
    logs = get_timeline(app, impl_id)
    last = next(l for l in logs if l['tipo_evento'] == 'tag_alterada')
    assert "Tag:" in last['detalhes'] and "→" in last['detalhes']
