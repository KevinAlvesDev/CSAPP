...
def validate_implantacao_belongs_to_context(impl_id: int, expected_context: str | None) -> bool:
    ...
    if expected_context is None:
        ...
        security_logger.debug(
            f"validate_implantacao_belongs_to_context: contexto None para impl_id={impl_id} "
            f"— acesso permitido (modo legado). Path: {request.path if request else 'N/A'}"
        )
    ...
    try:
        ...
        result = query_db("SELECT contexto FROM implantacoes WHERE id = %s", (impl_id,), one=True)



        if not result:

            security_logger.warning(f"Validação de contexto: implantação {impl_id} não encontrada")
        ...
        if expected_context == "onboarding":
            ...
            return bool(impl_context is None or impl_context == "onboarding")



        return bool(impl_context == expected_context)



    except Exception as e:

        security_logger.error(f"Erro ao validar contexto da implantação {impl_id}: {e}", exc_info=True)
...
def validate_checklist_item_belongs_to_context(item_id: int, expected_context: str | None) -> bool:
    ...
    if expected_context is None:
        ...
        security_logger.debug(
            f"validate_checklist_item_belongs_to_context: contexto None para item_id={item_id} "
            f"— acesso permitido (modo legado). Path: {request.path if request else 'N/A'}"
        )
    ...
    try:
        ...
        if not result:
            ...
            security_logger.warning(f"Validação de contexto: item {item_id} não encontrado ou sem implantação")
        ...
        security_logger.error(f"Erro ao validar contexto do item {item_id}: {e}", exc_info=True)
...
def validate_context_access(id_param: str = "impl_id", entity_type: str = "implantacao") -> Callable[[F], F]:
    ...
    def decorator(f: F) -> F:
        ...
        def decorated_function(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                return jsonify({"ok": False, "error": f"ID inválido: {entity_id}"}), 400
            ...
            if entity_type == "implantacao":
                ...
                is_valid = validate_implantacao_belongs_to_context(entity_id, expected_context)

            elif entity_type == "checklist_item":

                is_valid = validate_checklist_item_belongs_to_context(entity_id, expected_context)

            else:

                security_logger.error(
                    f"validate_context_access: entity_type desconhecido '{entity_type}' — acesso negado. "
                    f"User: {getattr(g, 'user_email', 'anonymous')}, path: {request.path}"
                )
            ...
            if not is_valid:
                ...
                return jsonify({"ok": False, "error": "Acesso negado: recurso não pertence ao contexto atual"}), 403
...
def add_context_filter_to_query(
    ...
    base_query: str, context: str | None, context_column: str = "contexto", params: list | None = None
    ...
    Example:
        ...
        query = "SELECT id, status FROM implantacoes WHERE status = %s"
        params = ['ativo']

        query, params = add_context_filter_to_query(query, 'grandes_contas', params=params)

        # query = "SELECT id, status FROM implantacoes WHERE status = %s AND contexto = %s"
...
def validate_context_value(context: str | None) -> str | None:
