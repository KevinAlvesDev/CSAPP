...
def get_implantacoes_with_progress(
    ...
    # Nosec rules here are safe because ALL dynamic formats are controlled internal constants

    # User input is purely bound to the parameterized queries args further below.
    ...
    """.format(
        ...
        child_active_check=child_active_check,

        active_leaf_check=active_leaf_check
    ...
    if date_type == "inicio":
        ...
        date_column = "COALESCE(NULLIF(CAST(i.data_finalizacao AS TEXT), ''), CAST(i.data_final_implantacao AS TEXT))"
...
def get_implantacao_with_details(implantacao_id: int) -> dict[str, Any] | None:
    ...
    return cast(dict[str, Any] | None, query_db(query, (implantacao_id,), one=True))
