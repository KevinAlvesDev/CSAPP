...
def log_function_call(logger: StructuredLogger = None):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # Log entrada
            logger.info(
                f"Entering {f.__name__}", function=f.__name__, args_count=len(args), kwargs_keys=list(kwargs.keys())
            )
            ...
            try:
                ...
                logger.info(f"Exiting {f.__name__} successfully", function=f.__name__, has_result=result is not None)
                ...
                # Log erro
                logger.error(
                    f"Error in {f.__name__}: {str(e)}", function=f.__name__, error_type=type(e).__name__, exc_info=True
                )
                raise
...
def log_external_api_call(api_name: str):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            logger.info(f"Calling external API: {api_name}", api_name=api_name, function=f.__name__)
...
class AuditLogger:
    ...
    def log_user_action(
        self, action: str, resource_type: str, resource_id: any = None, details: dict = None, success: bool = True
    ):
        """
        ...
        if success:
            self.logger.info(f"User action: {action} {resource_type}", **log_data)
        else:
            self.logger.warning(f"Failed user action: {action} {resource_type}", **log_data)

    def log_permission_check(self, resource_type: str, resource_id: any, required_permission: str, granted: bool):
        """
        ...
        if not granted:
            self.logger.warning(f"Permission denied: {required_permission} on {resource_type}", **log_data)
        else:
            self.logger.info(f"Permission granted: {required_permission} on {resource_type}", **log_data)
    ...
    def log_data_export(self, export_type: str, record_count: int, format: str = "csv"):
        ...
        """
        self.logger.info(
            f"Data export: {export_type}", export_type=export_type, record_count=record_count, format=format
        )
