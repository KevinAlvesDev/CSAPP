...
def receive_security_event():
    ...
    try:
        ...
        if not token:
            ...
            return jsonify({"error": "Token ausente", "message": "O campo SET é obrigatório"}), 400
        ...
        return jsonify({"error": "Erro interno", "message": "Erro ao processar evento de segurança"}), 500
...
def risc_status():
    ...
    """
    return jsonify({"status": "ok", "message": "RISC endpoint is operational", "endpoint": "/risc/events"}), 200
