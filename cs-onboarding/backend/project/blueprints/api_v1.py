...
from ..modules.implantacao.domain import (
...
def aplicar_oamd_implantacao(impl_id):
    ...
    if not verificar_permissao_por_contexto(g.perfil, "oamd.apply"):
        return jsonify({"ok": False, "error": "Sem permissão para aplicar dados do OAMD"}), 403
    ...
    try:
        ...
        if persist.get("nivel_receita_do_cliente"):
            ...
            updates["nivel_receita"] = str(persist["nivel_receita_do_cliente"])
        ...
        try:
            ...
            api_logger.warning(f"Erro ao limpar cache após aplicar OAMD: {e}", exc_info=True)
...
def save_carteira_content(impl_id):
    ...
    try:
        ...
        try:
            ...
            api_logger.warning(f"Erro ao limpar cache após salvar carteira: {e}", exc_info=True)
...
def create_aviso(impl_id):
    ...
    if not verificar_permissao_por_contexto(g.perfil, "avisos.create"):
        return jsonify({"ok": False, "error": "Sem permissão para criar avisos"}), 403
...
def update_aviso(impl_id, aviso_id):
    ...
    if not verificar_permissao_por_contexto(g.perfil, "avisos.edit"):
        return jsonify({"ok": False, "error": "Sem permissão para editar avisos"}), 403
...
def delete_aviso(impl_id, aviso_id):
    ...
    if not verificar_permissao_por_contexto(g.perfil, "avisos.delete"):
        return jsonify({"ok": False, "error": "Sem permissão para excluir avisos"}), 403
