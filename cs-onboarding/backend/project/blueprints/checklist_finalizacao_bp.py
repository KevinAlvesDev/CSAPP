...
def get_checklist(impl_id):
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Erro ao carregar checklist"}), 500
...
def toggle_item(item_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({"ok": False, "error": "Erro ao atualizar item"}), 500
...
def validar_checklist(impl_id):
    ...
    try:
        ...
        return jsonify({"ok": True, "validado": validado, "mensagem": mensagem}), 200

    except Exception as e:
        logger.error(f"Erro ao validar checklist da implantação {impl_id}: {e}", exc_info=True)
        return jsonify({"ok": False, "error": "Erro ao validar checklist"}), 500
...
def get_templates():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Erro ao carregar templates"}), 500
...
def criar_checklist(impl_id):
    ...
    try:
        ...
        if success:
            return jsonify({"ok": True, "message": "Checklist criado com sucesso"}), 200
        else:
            return jsonify({"ok": False, "error": "Erro ao criar checklist"}), 500
