...
def soft_delete(table: str, record_id: int, id_column: str = "id") -> bool:
    ...
    try:
        ...
        current_app.logger.error(f"Error soft deleting {table} ID {record_id}: {e}", exc_info=True)
...
def restore(table: str, record_id: int, id_column: str = "id") -> bool:
    ...
    try:
        ...
        current_app.logger.error(f"Error restoring {table} ID {record_id}: {e}", exc_info=True)
...
def hard_delete(table: str, record_id: int, id_column: str = "id") -> bool:
    ...
    try:
        ...
        current_app.logger.error(f"Error hard deleting {table} ID {record_id}: {e}", exc_info=True)
...
def get_deleted_records(table: str, limit: int = 100) -> list:
    ...
    try:
        ...
        current_app.logger.error(f"Error getting deleted records from {table}: {e}", exc_info=True)
...
def cleanup_old_deleted_records(table: str, days: int = 30) -> int:
    ...
    try:
        ...
        if rows_affected and rows_affected > 0:
            ...
            return int(rows_affected) if rows_affected else 0
        ...
        current_app.logger.error(f"Error cleaning up old deleted records from {table}: {e}", exc_info=True)
...
def exclude_deleted(query: str) -> str:
    ...
    Exemplo:
        ...
        query = "SELECT id, usuario_cs FROM implantacoes WHERE usuario_cs = %s"

        query = exclude_deleted(query)

        # Resultado: "SELECT id, usuario_cs FROM implantacoes WHERE usuario_cs = %s AND deleted_at IS NULL"
