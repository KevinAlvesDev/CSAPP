...
def dashboard():
    ...
    session_key = "ongoing_dashboard_filters"
    ...
    contexto_atual = resolve_context(getattr(g, "modulo_atual", "ongoing"))
    ...
    if date_type_request_param is None and not (start_date or end_date) and date_type == "criacao":  # pragma: no cover
        date_type = ""  # pragma: no cover
    if (
    ...
    try:
        ...
        current_app.logger.error(f"Erro ao buscar tags metrics: {e}", exc_info=True)
    ...
    try:
        ...
        # Passando context='ongoing' para filtrar apenas este modulo
        ...
        if sort_days in ["asc", "desc"]:
            ...
            try:
                ...
                current_app.logger.warning(f"Falha ao ordenar implantações por dias passados: {e}", exc_info=True)
...
@ongoing_bp.route("/implantacao/<int:impl_id>")
...
def ver_implantacao(impl_id):
    ...
    try:
        ...
        logger.error(f"ID de implantacao invalido: {e!s}", exc_info=True)

        flash(f"ID de implantacao invalido: {e!s}", "error")

        return redirect(url_for("grandes_contas.dashboard", context=getattr(g, "modulo_atual", None)))
    ...
    try:
        ...
        return render_template("pages/ongoing/implantacao_detalhes.html", **context_data)



    except ValueError as e:

        logger.warning(f"Acesso negado a implantacao {impl_id}: {e!s}", exc_info=True)
