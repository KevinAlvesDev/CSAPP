...
ongoing_actions_bp = Blueprint("ongoing_actions", __name__)


@ongoing_actions_bp.route("/criar_implantacao", methods=["POST"])
@login_required
@permission_required(PERFIS_COM_CRIACAO)
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
@audit(action="CREATE_IMPLANTACAO_ONGOING", target_type="implantacao")
def criar_implantacao():
    return handle_criar_implantacao(
        dashboard_endpoint="ongoing.dashboard",
        contexto="ongoing",
...
@ongoing_actions_bp.route("/criar_implantacao_modulo", methods=["POST"])
@login_required
@permission_required(PERFIS_COM_CRIACAO)
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
@audit(action="CREATE_IMPLANTACAO_MODULO_ONGOING", target_type="implantacao")
...
@ongoing_actions_bp.route("/iniciar_implantacao", methods=["POST"])
@login_required
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
def iniciar_implantacao():
    ...
    return handle_iniciar_implantacao(
        dashboard_endpoint="ongoing.dashboard",
        detail_endpoint="ongoing.ver_implantacao",
        success_message="Implantação iniciada com sucesso!",
        clear_dashboard=True,
    )


@ongoing_actions_bp.route("/desfazer_inicio_implantacao", methods=["POST"])
@login_required
def desfazer_inicio_implantacao():
    return handle_desfazer_inicio_implantacao(clear_dashboard=True)

@ongoing_actions_bp.route("/agendar_implantacao", methods=["POST"])
@login_required
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
def agendar_implantacao():
    return handle_agendar_implantacao(dashboard_endpoint="ongoing.dashboard", clear_dashboard=True)


@ongoing_actions_bp.route("/marcar_sem_previsao", methods=["POST"])
@login_required
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
def marcar_sem_previsao():
    return handle_marcar_sem_previsao(dashboard_endpoint="ongoing.dashboard", clear_dashboard=True)


@ongoing_actions_bp.route("/finalizar_implantacao", methods=["POST"])
@login_required
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
@audit(action="FINALIZE_IMPLANTACAO_ONGOING", target_type="implantacao")
...
@ongoing_actions_bp.route("/parar_implantacao", methods=["POST"])
@login_required
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
def parar_implantacao():
    ...
    return handle_parar_implantacao(
        detail_endpoint="ongoing.ver_implantacao",
        clear_dashboard=True,
        success_message='Implantação marcada como "Parada" (Ongoing).',
    )


@ongoing_actions_bp.route("/retomar_implantacao", methods=["POST"])
@login_required
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
...
@ongoing_actions_bp.route("/reabrir_implantacao", methods=["POST"])
@login_required
@limiter.limit("50 per minute", key_func=lambda: g.user_email or get_remote_address())
...
@ongoing_actions_bp.route("/atualizar_detalhes_empresa", methods=["POST"])
@login_required
...
@ongoing_actions_bp.route("/remover_plano_implantacao", methods=["POST"])
@login_required
...
@ongoing_actions_bp.route("/transferir_implantacao", methods=["POST"])
@login_required
@permission_required(PERFIS_COM_GESTAO)
@limiter.limit("30 per minute", key_func=lambda: g.user_email or get_remote_address())
@audit(action="TRANSFER_IMPLANTACAO_ONGOING", target_type="implantacao")
...
@ongoing_actions_bp.route("/excluir_implantacao", methods=["POST"])
@login_required
@limiter.limit("100 per minute", key_func=lambda: g.user_email or get_remote_address())
@audit(action="DELETE_IMPLANTACAO_ONGOING", target_type="implantacao")
...
@ongoing_actions_bp.route("/cancelar_implantacao", methods=["POST"])
@login_required
@audit(action="CANCEL_IMPLANTACAO_ONGOING", target_type="implantacao")
...
@ongoing_actions_bp.route("/api/implantacao/<int:implantacao_id>/jira-issues", methods=["GET"])
@login_required
def get_jira_issues(implantacao_id):
    return handle_get_jira_issues(implantacao_id, log_label="[Ongoing]")

@ongoing_actions_bp.route("/api/implantacao/<int:implantacao_id>/jira-issues", methods=["POST"])
@login_required
...
@ongoing_actions_bp.route("/api/implantacao/<int:implantacao_id>/jira-issues/fetch", methods=["POST"])
@login_required
def fetch_jira_issue_action(implantacao_id):
    return handle_fetch_jira_issue(implantacao_id)

@ongoing_actions_bp.route(
