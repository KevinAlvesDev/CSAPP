...
from ....common.exceptions import ResourceNotFoundError, AuthorizationError, BusinessRuleError
...
def _get_implantacao_and_validate_access(
    ...
    implantacao = query_db(
        "SELECT * FROM implantacoes WHERE id = %s",
        (impl_id,),
        one=True,
    )
    ...
    if not is_owner and not is_manager:
        ...
        if implantacao.get("status") == "nova":
            ...
            raise BusinessRuleError("Esta implantação ainda não foi iniciada.")

        raise AuthorizationError("Permissão negada. Implantação não encontrada ou não pertence a você.")
