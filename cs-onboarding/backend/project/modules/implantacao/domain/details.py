...
from ....common.exceptions import ResourceNotFoundError, AuthorizationError, ValidationError
...
def _format_implantacao_dates(implantacao):
    ...
    def _fmt_d(key):
        raw = implantacao.get(key)
        if not raw:
            return None
        result = format_date_br(raw, False)
        return None if result == "N/A" else result

    implantacao["data_inicio_efetivo_fmt_d"] = _fmt_d("data_inicio_efetivo")

    implantacao["data_finalizacao_fmt_d"] = _fmt_d("data_finalizacao")

    implantacao["data_inicio_producao_fmt_d"] = _fmt_d("data_inicio_producao")

    implantacao["data_final_implantacao_fmt_d"] = _fmt_d("data_final_implantacao")

    implantacao["data_previsao_termino_fmt_d"] = _fmt_d("data_previsao_termino")
...
def _get_comentarios_bulk(
    ...
    comentarios_map: dict[str, list[dict[str, Any]]] = {}
    ...
    try:
        ...
        comentarios_h_raw = (
            ...
            SELECT c.*,
                   ...
                   COALESCE(u.nome, c.usuario_cs) as usuario_nome,
            ...
            LEFT JOIN perfil_usuario u ON c.usuario_cs = u.usuario
        ...
        logger.warning(f"Erro ao buscar comentarios_h em bulk para implantação {impl_id}: {e}", exc_info=True)
...
def _get_tarefas_and_comentarios(
    ...
    try:
        ...
        logger.warning(f"Erro ao carregar fases para implantação {impl_id}: {e}", exc_info=True)
...
def get_implantacao_details(
    ...
    try:
        ...
        logger.warning(f"Falha ao limpar cache da implantação {impl_id} antes de carregar detalhes: {e}", exc_info=True)
    ...
    try:
        ...
        logger.warning(f"Acesso negado à implantação {impl_id}: {e!s}", exc_info=True)
    ...
    try:
        ...
        all_cs_users = query_db(
            ...
            SELECT u.usuario as usuario, u.nome
    ...
    if (
        ...
        try:
            ...
            plano_atual_id = plano_atual.get("id") if plano_atual else None

            if not plano_atual_id and plano_atual:

                plano_atual_id = plano_atual.get("codigo")
            ...
            if plano_atual_id:
                ...
                with db_connection() as (conn, db_type):
                    ...
                    cursor.execute(sql_update, (plano_atual_id, impl_id))

                    conn.commit()

                implantacao["plano_sucesso_id"] = plano_atual_id

                logger.warning(

                    f"Implantação {impl_id} estava sem plano_sucesso_id. Relink automático para plano {plano_atual_id}."

                )

        except Exception as e:

            logger.warning(f"Falha ao tentar relink automático do plano da implantação {impl_id}: {e}", exc_info=True)
        ...
        try:
            ...
            if plano_atual and plano_atual.get("processo_id") is None:
                ...
                if not instancia:
                    ...
                    plano_atual_id = plano_atual.get("id") or plano_atual.get("codigo")
                ...
                instancia_id = instancia.get("id") or instancia.get("codigo")
                ...
                with db_connection() as (conn, db_type):
                    ...
                    cursor.execute(sql_update, (instancia_id, impl_id))

                    conn.commit()

                implantacao["plano_sucesso_id"] = instancia_id

                logger.warning(

                    f"Implantação {impl_id} apontava para template. Relink automático para plano {instancia_id}."

                )

        except Exception as e:

            logger.warning(f"Falha ao auto-corrigir plano da implantação {impl_id}: {e}", exc_info=True)
    ...
    try:
        ...
        logger.warning(f"Erro ao buscar plano de sucesso {success_plan_id}: {e}", exc_info=True)
    ...
    try:
        ...
        if not checklist_flat and implantacao.get("plano_sucesso_id") and plano_sucesso_info and not plano_historico_id:
            ...
            try:
                ...
                base_dt = datetime_dt.now(timezone.utc)
                ...
                try:
                    ...
                    logger.warning(f"Falha ao limpar cache da implantação {impl_id} após auto-reparo do checklist: {e}", exc_info=True)
    ...
    planos_lista: list[dict[str, Any]] = []
    ...
    try:
        ...
        logger.warning(f"Erro ao buscar lista de planos da implantação {impl_id}: {e}", exc_info=True)
    ...
    try:
        ...
        logger.warning(f"Erro ao verificar permissões de checklist da implantação {impl_id}: {e}", exc_info=True)
...
def _get_timeline_logs(impl_id):
    ...
    logs_timeline = query_db(
        ...
        """ SELECT tl.*, COALESCE(u.nome, tl.usuario_cs) as usuario_nome

            FROM timeline_log tl LEFT JOIN perfil_usuario u ON tl.usuario_cs = u.usuario
...
def atualizar_detalhes_empresa_service(implantacao_id, usuario_cs_email, user_perfil_acesso, campos):
    ...
    if not impl:
        ...
        raise ResourceNotFoundError("Implantação", implantacao_id)
    ...
    if not (is_owner or is_manager):
        ...
        raise AuthorizationError("Permissão negada para atualizar detalhes desta empresa.")
    ...
    if not is_valid:
        ...
        raise ValidationError(error_message, details=validation_errors)
    ...
    allowed_fields = {
        ...
        "valor_atribuido",  # Valor atribu?do (R$)
    ...
    # Detectar coluna varchar excedida com base no schema real do banco
    try:
        schema_rows = query_db(
            """
            SELECT column_name, character_maximum_length
            FROM information_schema.columns
            WHERE table_schema = 'public'
              AND table_name = 'implantacoes'
              AND character_maximum_length IS NOT NULL
            """,
            (),
        ) or []
        max_len_map = {
            r.get("column_name"): int(r.get("character_maximum_length"))
            for r in schema_rows
            if r and r.get("column_name") and r.get("character_maximum_length")
        }
        for k, v in campos.items():
            limit = max_len_map.get(k)
            if limit and isinstance(v, str) and len(v) > limit:
                sample = (v[:80] + "...") if len(v) > 80 else v
                raise ValidationError(
                    f"Campo '{k}' excedeu o limite do banco ({limit}). Tamanho: {len(v)}. Valor: {sample}"
                )
    except ValidationError:
        raise
    except Exception as e:
        logger.warning(f"Falha ao validar tamanho de colunas no schema: {e}", exc_info=True)
    ...
    if campos.get("data_inicio_efetivo"):
        ...
        try:
            ...
            current_app.logger.warning(f"Erro ao recalcular previsão de término: {e}", exc_info=True)
    ...
    with db_connection() as (conn, db_type):
        ...
        try:
            ...
            current_app.logger.error(f"Transaction failed and rolled back for implantacao {implantacao_id}: {e}", exc_info=True)
