...
def _clear_implantacao_related_caches(implantacao_id: int, usuario_cs_email: str, impl_owner_email: str | None = None) -> None:

    """Limpa os caches relacionados a uma implantação e seus usuários."""
    ...
    try:
        ...
        if impl_owner_email and impl_owner_email != usuario_cs_email:

            clear_user_cache(impl_owner_email)

    except Exception as e:

        current_app.logger.warning(f"Falha ao limpar cache da implantação {implantacao_id}: {e}", exc_info=True)
...
def iniciar_implantacao_service(implantacao_id, usuario_cs_email):
    ...
    if not is_admin and impl.get("usuario_cs") != usuario_cs_email:
        ...
        raise AuthorizationError("Operação negada. Implantação não pertence a você.")



    if impl.get("status") not in ["nova", "futura", "sem_previsao"]:

        raise BusinessRuleError(f'Operação negada. Implantação com status "{impl.get("status")}" não pode ser iniciada.', details={"status": impl.get("status")})
    ...
    _clear_implantacao_related_caches(implantacao_id, usuario_cs_email, impl.get("usuario_cs"))
    ...
    try:
        ...
        current_app.logger.warning(f"Falha ao emitir evento ImplantacaoIniciada: {e}", exc_info=True)
...
def desfazer_inicio_implantacao_service(implantacao_id, usuario_cs_email):
    ...
    if impl.get("status") != "andamento":
        ...
        raise BusinessRuleError(
            ...
            details={"status": impl.get("status")}
...
def agendar_implantacao_service(implantacao_id, usuario_cs_email, data_prevista_iso):
    ...
    if impl.get("status") not in ["nova", "sem_previsao", "futura"]:
        ...
        raise BusinessRuleError('Apenas implantações "Novas", "Sem previsão" ou "Futuras" podem ser agendadas.', details={"status": impl.get("status")})
...
def marcar_sem_previsao_service(implantacao_id, usuario_cs_email):
    ...
    if impl.get("status") != "nova":
        ...
        raise BusinessRuleError('Apenas implantações "Novas" podem ser marcadas como Sem previsão.', details={"status": impl.get("status")})
    ...
    if not updated:
        ...
        raise DatabaseError("Falha ao atualizar status para Sem previsão.")
...
def finalizar_implantacao_service(implantacao_id, usuario_cs_email, data_final_iso):
    ...
    if impl.get("usuario_cs") != usuario_cs_email:
        ...
        raise AuthorizationError("Permissão negada. Esta implantação não pertence a você.")
    ...
    try:
        ...
        from ....modules.checklist.application.checklist_finalizacao_service import validar_checklist_completo
        ...
        if not validado:
            ...
            raise BusinessRuleError(f"Checklist de Finalização incompleto: {mensagem}", details={"mensagem": mensagem})
    ...
    if plano_id:
        ...
        if total_pendentes > 0:
            ...
            raise BusinessRuleError(
                ...
                details={"total_pendentes": total_pendentes}
    ...
    try:
        ...
        current_app.logger.warning(f"Falha ao emitir evento ImplantacaoFinalizada: {e}", exc_info=True)
...
def parar_implantacao_service(implantacao_id, usuario_cs_email, user_perfil_acesso, data_parada_iso, motivo):
    ...
    if impl.get("status") != "andamento":
        ...
        raise BusinessRuleError('Apenas implantações "Em Andamento" podem ser marcadas como "Parada".', details={"status": impl.get("status")})
...
def retomar_implantacao_service(implantacao_id, usuario_cs_email, user_perfil_acesso):
    ...
    if impl.get("status") != "parada":
        ...
        raise BusinessRuleError('Apenas implantações "Paradas" podem ser retomadas.', details={"status": impl.get("status")})
...
def reabrir_implantacao_service(implantacao_id, usuario_cs_email):
    ...
    if impl.get("status") != "finalizada":
        ...
        raise BusinessRuleError('Apenas implantações "Finalizadas" podem ser reabertas.', details={"status": impl.get("status")})
...
def desfazer_cancelamento_implantacao_service(implantacao_id, usuario_cs_email, user_perfil_acesso):
    ...
    if not (is_owner or is_manager):
        ...
        raise AuthorizationError("Permissão negada para esta ação.")
