...
def _search_by_infra(infra_req):
    ...
    if emp_int:
        ...
        where_clause = "ef.empresazw = %s"

        params = [cast(Any, emp_int)]

    else:

        # Busca por nome (LIKE)

        where_clause = "LOWER(ef.nomeempresazw) LIKE %s"

        params = [cast(Any, f"%{str(infra_req).lower()}%")]
    ...
    try:
        ...
        api_logger.warning(f"Erro ao buscar por infra '{infra_req}': {e}", exc_info=True)
...
def _search_by_id_favorecido(id_favorecido):
    ...
    params: tuple[Any, ...] = (id_favorecido,)
    ...
    try:
        ...
        where_clause = "ef.codigofinanceiro = %s"

        query = build_oamd_query(where_clause)

        results = query_db(query, params) or []
        ...
        api_logger.warning(f"Erro ao buscar por ef.codigofinanceiro: {e}", exc_info=True)
    ...
    try:
        ...
        api_logger.warning(f"Erro ao buscar por ef.codigo: {e}", exc_info=True)
