...
def search_issues_by_context(implantacao_data, max_results=50, extra_keys=None):
    ...
    if extra_keys and isinstance(extra_keys, list) and len(extra_keys) > 0:
        ...
        if safe_keys:
            ...
            try:
                ...
                logger.error(f"Erro ao buscar extra keys: {e}", exc_info=True)
    ...
    if company_name:
        ...
        try:
            ...
            logger.error(f"Erro ao buscar contexto da empresa: {e}", exc_info=True)
...
def create_jira_issue(implantacao_data, issue_data, files=None):
    ...
    try:
            ...
            if files:
                ...
                for file_obj in files:
                    ...
                    try:
                        ...
                        logger.error(f"Erro anexando arquivo {file_obj}: {e_att}", exc_info=True)
        ...
        logger.error(f"Exceção create_jira_issue: {e}", exc_info=True)
...
def attach_file_to_issue(issue_key, file_storage):
    ...
    try:
        ...
        logger.error(f"Exceção attach_file_to_issue: {e}", exc_info=True)
...
def get_issue_details(issue_key):
    ...
    try:
        ...
        logger.error(f"Erro ao buscar issue {issue_key}: {e}", exc_info=True)
...
def save_jira_link(implantacao_id, jira_key, user_email):
    ...
    try:
        ...
        logger.error(f"Erro critical salvar link Jira: {e}", exc_info=True)
...
def remove_jira_link(implantacao_id, jira_key):
    ...
    try:
        ...
        logger.error(f"Erro ao remover link Jira: {e}", exc_info=True)
...
def get_linked_jira_keys(implantacao_id):
    ...
    try:
        ...
        logger.warning(f"Erro ao buscar links Jira (DB): {e}", exc_info=True)
