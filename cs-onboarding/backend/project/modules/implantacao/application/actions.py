...
def handle_iniciar_implantacao(
    *,
    ...
    dest_url_fallback = url_for(dashboard_endpoint)
    if redirect_to_fallback == "detalhes":
        dest_url_fallback = url_for(detail_endpoint, impl_id=implantacao_id)
    ...
    try:
        ...
        return redirect(url_for(detail_endpoint, impl_id=implantacao_id))
...
def handle_agendar_implantacao(*, dashboard_endpoint: str, clear_dashboard: bool) -> object:
    ...
    return redirect(url_for(dashboard_endpoint, refresh="1"))
...
def handle_finalizar_implantacao(*, dashboard_endpoint: str, detail_endpoint: str, clear_dashboard: bool) -> object:
    ...
    dest_url = url_for(dashboard_endpoint)
    if redirect_target == "detalhes":
        dest_url = url_for(detail_endpoint, impl_id=implantacao_id)
...
def handle_reabrir_implantacao(*, dashboard_endpoint: str, detail_endpoint: str, clear_dashboard: bool) -> object:
    ...
    try:
        ...
        flash(str(e), "error")
        return redirect(request.referrer or url_for(dashboard_endpoint))
    except Exception as e:
...
def handle_get_jira_issues(implantacao_id: int, *, log_label: str = "") -> object:
    try:
        ...
        app_logger.error(f"Erro na rota jira-issues {log_label}: {e}")
