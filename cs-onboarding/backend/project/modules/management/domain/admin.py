...
def atualizar_perfil_usuario_service(usuario_alvo, novo_perfil, usuario_admin, context=None):
    ...
    if usuario_alvo == usuario_admin:
        if usuario_alvo != ADMIN_EMAIL or usuario_alvo == MASTER_ADMIN_EMAIL:
            raise ValueError("N?o pode alterar o seu pr?prio perfil por esta interface.")

    if MASTER_ADMIN_EMAIL and usuario_alvo == MASTER_ADMIN_EMAIL and novo_perfil != PERFIL_ADMIN:
    ...
    # Executar atualização (perfil_acesso vai para perfil_usuario_contexto via set_user_role)
    ...
    try:
        ...
        management_logger.warning(f"Falha ao invalidar cache de perfil para {usuario_alvo}: {e}", exc_info=True)
...
def excluir_usuario_service(usuario_alvo, usuario_admin):
    ...
    # Excluir contextos de perfil (CONTROL)

    execute_db("DELETE FROM perfil_usuario_contexto WHERE usuario = %s", (usuario_alvo,))

    # Desativar usuário no OAMD (não deletar — dado compartilhado entre sistemas)

    execute_db("UPDATE perfil_usuario SET ativo = false WHERE usuario = %s", (usuario_alvo,))
    ...
    try:
        ...
        management_logger.warning(f"Falha ao limpar cache do perfil do usuário {usuario_alvo} após exclusão: {e}", exc_info=True)
