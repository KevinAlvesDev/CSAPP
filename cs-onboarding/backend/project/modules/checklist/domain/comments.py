    ...
    with db_transaction_with_lock() as (conn, cursor, db_type):
        ...
        if db_type == "postgres":
            ...
            try:
                ...
                logger.warning(f"Falha ao garantir colunas da tabela comentarios_h (postgres): {e}", exc_info=True)
            ...
            try:
                ...
                logger.warning(f"Falha ao garantir colunas da tabela comentarios_h (sqlite): {e}", exc_info=True)
        ...
        try:
            ...
            logger.warning(f"Erro ao invalidar cache após criar comentário: {e}", exc_info=True)
        ...
        try:
            ...
            logger.warning(f"Falha ao emitir evento ChecklistComentarioAdicionado: {e}", exc_info=True)
...
def update_comment_service(comentario_id, novo_texto, usuario_email, is_manager):
    ...
    try:
        ...
        logger.warning(f"Erro ao invalidar cache após editar comentário: {e}", exc_info=True)
...
def excluir_comentario_service(comentario_id, usuario_email, is_manager):
    ...
    if item_info:
        ...
        try:
            ...
            logger.warning(f"Falha ao registrar timeline de exclusão de comentário: {e}", exc_info=True)
        ...
        try:
            ...
            logger.warning(f"Erro ao invalidar cache após excluir comentário: {e}", exc_info=True)
