...
def _now_utc() -> datetime:
    """Retorna agora em UTC, com fallback para datetime.now() em testes."""
    try:
...
def _descendants_sql(db_type: str) -> str:
    """SQL template para CTE recursiva de descendentes."""
    if db_type == "postgres":
        return """
...
def _ancestors_sql(db_type: str) -> str:
    """SQL template para CTE recursiva de ancestrais."""
    if db_type == "postgres":
        return """
...
def _get_descendant_ids(cursor, db_type, item_id):
    ...
    descendants_query = _descendants_sql(db_type)
...
def toggle_item_status(item_id, new_status, usuario_email=None):
    ...
    data_conclusao = _now_utc() if new_status else None
    ...
    with db_transaction_with_lock() as (conn, cursor, db_type):
        ...
        if descendant_ids:
            ...
            cursor.execute(status_query, descendant_ids)  # nosec B608
            ...
            if descendant_ids:
                ...
                cursor.execute(update_sql, params)  # nosec B608
            ...
            if history_entries:
                ...
                try:
                    ...
                    logger.warning(f"Falha ao inserir histórico de status: {e}", exc_info=True)
        ...
        ancestors_query = _ancestors_sql(db_type)
        ...
        for ancestor_id in ancestor_ids:
            ...
            if curr_anc_status != new_anc_status_str:
                ...
                try:
                    ...
                    logger.warning(f"Falha ao registrar histórico de status do item ancestral {ancestor_id}: {e}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Failed to log timeline inside service: {e}", exc_info=True)
        ...
        if new_status:
            ...
            try:
                ...
                logger.warning(f"Falha ao emitir evento ChecklistItemConcluido para item {item_id}: {e}", exc_info=True)
...
def update_item_responsavel(item_id, novo_responsavel, usuario_email=None):
    ...
    with db_transaction_with_lock() as (conn, cursor, db_type):
        ...
        try:
            ...
            logger.warning(f"Falha ao registrar histórico de responsável do item {item_id}: {e}", exc_info=True)
        ...
        try:
            ...
            logger.warning(f"Falha ao registrar timeline de alteração de responsável do item {item_id}: {e}", exc_info=True)
...
def delete_checklist_item(item_id, usuario_email=None, is_manager=False):
    ...
    with db_transaction_with_lock() as (_conn, cursor, db_type):
        ...
        try:
            ...
            # 3. Identificar todos os descendentes para exclus?o
            ...
            if ids_to_delete:
                # Validar que todos os IDs s?o inteiros (seguran?a contra SQL injection)
                try:
                    ids_to_delete = [int(id_val) for id_val in ids_to_delete]
                except (ValueError, TypeError) as e:
                    raise ValueError(f"IDs inv?lidos detectados: {e}") from e
            ...
            if parent_id:
                ...
                if db_type == "postgres":
                    ...
                    if total > 0:
                        ...
                        new_date = _now_utc() if new_status else None

                        cursor.execute(

                            "UPDATE checklist_items SET completed = %s, status = %s, data_conclusao = %s, updated_at = %s WHERE id = %s",

                            (new_status, new_status_str, new_date, _now_utc(), parent_id),
                ...
                while curr_parent:
                    ...
                    if total_f > 0:
                        ...
                        new_date = _now_utc() if new_st else None
                        ...
                        if db_type == "postgres":
                            ...
                            cursor.execute(
                                ...
                                (new_st, new_status_str, new_date, _now_utc(), curr_parent),
...
def set_item_dispensa(item_id, dispensar=True, motivo="", usuario_email=None, is_manager=False):
    ...
    with db_transaction_with_lock() as (conn, cursor, db_type):
        ...
        try:
            ...
            logger.warning(f"Falha ao registrar timeline de dispensa: {e}", exc_info=True)
...
def atualizar_prazo_item(item_id, nova_data_iso, usuario_email):
    ...
    with db_transaction_with_lock() as (conn, cursor, db_type):
        ...
        nova_dt = cast(datetime, adjust_to_business_day(nova_dt))
        ...
        try:
            ...
            logger.warning(f"Falha ao registrar timeline de alteração de prazo do item {item_id}: {e}", exc_info=True)
...
def move_item(item_id, new_parent_id=None, new_order=None, usuario_email=None):
    ...
    with db_transaction_with_lock() as (conn, cursor, db_type):
        ...
        try:
            ...
            logger.warning(f"Erro ao logar timeline: {e}", exc_info=True)
