...
def _invalidar_cache_progresso_local(impl_id):
    ...
    try:
        ...
        logger.warning(f"Erro ao invalidar cache de progresso para impl_id {impl_id}: {e}", exc_info=True)
...
def listar_usuarios_cs():
    ...
    rows = (
        ...
        query_db(
            ...
            SELECT u.usuario AS usuario, COALESCE(u.nome, u.usuario) AS nome
...
def plano_permite_excluir_tarefas(item_id):
    ...
    try:
        ...
        logger.warning(f"Erro ao verificar permissão do plano para item {item_id}: {e}", exc_info=True)
