...
from .users import verificar_usuario_existe, obter_perfil_usuario
...
def excluir_usuario_service_v2(usuario_alvo, usuario_admin):
    ...
    stats = query_db(
        "SELECT COUNT(*) as total FROM implantacoes WHERE usuario_cs = %s",
        (usuario_alvo,),
        one=True
    ) or {'total': 0}
    ...
    management_logger.info(
        ...
        f"(implantações vinculadas removidas: {total_implantacoes})"
    )
    
    return {
        'implantacoes_excluidas': total_implantacoes,
        'foto_url': foto_url
    }
