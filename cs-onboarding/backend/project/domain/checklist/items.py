...
def move_item(item_id, new_parent_id=None, new_order=None, usuario_email=None):
    ...
    if new_parent_id is not None and new_parent_id != -1:
        try:
            ...
            raise ValueError("new_parent_id deve ser um inteiro válido")
    ...
    if new_order is not None:
        try:
            ...
            raise ValueError("new_order deve ser um inteiro válido")
    ...
    with db_transaction_with_lock() as (conn, cursor, db_type):
        ...
        logger.info(f"Item {item_id} movido: parent_id={current_parent_id}->{new_parent_id}, ordem={current_order}->{new_order}")
