...
from ..db import execute_db, query_db, db_connection
...
def marcar_item_checklist(item_id: int, concluido: bool, usuario: str, 
        ...
        with db_connection() as (conn, db_type):
            ...
            if db_type == 'postgres':
                cursor.execute("""
                    ...
                    WHERE id = %s
                    """,
                    (concluido, data_conclusao, usuario_conclusao, evidencia_tipo,
                     evidencia_conteudo, evidencia_url, observacoes, datetime.now(), item_id)
                )
            else:  # SQLite
                cursor.execute("""
                    UPDATE checklist_finalizacao_items
                    ...
                    WHERE id = ?
                    """,
                    (1 if concluido else 0, data_conclusao, usuario_conclusao, evidencia_tipo,
                     evidencia_conteudo, evidencia_url, observacoes, datetime.now(), item_id)
                )
