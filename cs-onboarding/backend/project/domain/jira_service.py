...
def save_jira_link(implantacao_id, jira_key, user_email):
    ...
    try:
        ...
        if use_sqlite:
            try:
                cur.execute("INSERT INTO implantacao_jira_links (implantacao_id, jira_key, vinculado_por) VALUES (?, ?, ?)", 
            ...
            cur.execute("INSERT INTO implantacao_jira_links (implantacao_id, jira_key, vinculado_por) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING", 
                        (implantacao_id, jira_key, user_email))
            conn.commit()
...
def remove_jira_link(implantacao_id, jira_key):
    ...
    try:
        ...
        if use_sqlite:
            cur.execute("DELETE FROM implantacao_jira_links WHERE implantacao_id = ? AND jira_key = ?", 
                        (implantacao_id, jira_key))
        else:
            cur.execute("DELETE FROM implantacao_jira_links WHERE implantacao_id = %s AND jira_key = %s", 
                        (implantacao_id, jira_key))
