...
def get_valid_tags():
    """
    Retorna tags válidas do banco de dados.
    Fallback para tags hardcoded se tabela não existir.
    """
    try:
        tags = query_db("""
...
def validar_estrutura_hierarquica(estrutura: Dict) -> bool:
    ...
    # Carregar tags válidas do banco de dados
    ...
    for i, fase in enumerate(fases):
        ...
        for j, grupo in enumerate(grupos):
            ...
            if not isinstance(tarefas, list):
                raise ValidationError(f"'tarefas' do grupo '{grupo['nome']}' deve ser uma lista")

            for k, tarefa in enumerate(tarefas):
                if not isinstance(tarefa, dict):
                    raise ValidationError(f"Tarefa {k + 1} do grupo '{grupo['nome']}' deve ser um dicionário")

                if "nome" not in tarefa or not tarefa["nome"]:
                    raise ValidationError(f"Tarefa {k + 1} do grupo '{grupo['nome']}' deve ter um nome")
                ...
                if not isinstance(subtarefas, list):
                    raise ValidationError(f"'subtarefas' da tarefa '{tarefa['nome']}' deve ser uma lista")
                ...
                for idx, subtarefa in enumerate(subtarefas):
                    ...
                    if "nome" not in subtarefa or not subtarefa["nome"]:
                        raise ValidationError(f"Subtarefa {idx + 1} da tarefa '{tarefa['nome']}' deve ter um nome")
...
def _validar_items_recursivo(items: List[Dict], titles_set: set, depth: int, max_depth: int = 100):
    ...
    for i, item in enumerate(items):
        ...
        tag = item.get("tag")
        if tag and tag not in VALID_TAGS:
            raise ValidationError(f"Item '{title}' tem tag inválida '{tag}'. Tags permitidas: {', '.join(VALID_TAGS)}")
