...
from ...common.query_helpers import get_implantacao_with_details
...
from ...constants import PERFIS_COM_GESTAO, MODULO_PENDENCIAS
...
def build_tarefas_agrupadas_optimized(
    tree: Dict[str, Any],
    comentarios_map: Dict[int, List[Dict]]
    ...
    for fase in fases:
        ...
        for grupo in grupos:
            ...
            if modulo_nome in tarefas_agrupadas_treinamento:
                ...
                try:
                    modulo_ordem_map[modulo_nome] = min([t.get('ordem', 0) for t in tarefas_agrupadas_treinamento[modulo_nome]])
                except:
...
def get_implantacao_details_v2(
    impl_id: int,
    ...
    (tarefas_agrupadas_obrigatorio, 
     ordered_treinamento, 
     tarefas_agrupadas_pendencias, 
     todos_modulos_lista) = build_tarefas_agrupadas_optimized(tree, comentarios_map)
    ...
    from ...constants import (
        CARGOS_RESPONSAVEL, NIVEIS_RECEITA, SEGUIMENTOS_LIST,
        TIPOS_PLANOS, MODALIDADES_LIST, HORARIOS_FUNCIONAMENTO,
        FORMAS_PAGAMENTO, SISTEMAS_ANTERIORES, RECORRENCIA_USADA,
        SIM_NAO_OPTIONS, JUSTIFICATIVAS_PARADA
    )
