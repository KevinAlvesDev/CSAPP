...
def detect_smtp_settings(user_email):
    ...
    COMMON: dict[str, tuple[str, int, bool, bool]] = {
...
def load_smtp_settings(user_email):
    ...
    try:
        ...
        app_logger.error(f"Erro ao carregar settings SMTP para {user_email}: {e}", exc_info=True)
...
def save_smtp_settings(user_email, data):
    ...
    try:
        ...
        app_logger.error(f"Erro ao salvar settings SMTP para {user_email}: {e}", exc_info=True)
...
def test_smtp_connection(settings, plain_password):
    ...
    try:
        ...
        if use_ssl:
            ...
            context = smtplib.ssl.create_default_context() # type: ignore[attr-defined]

            server = smtplib.SMTP_SSL(host, port, context=context)

        else:

            server = smtplib.SMTP(host, port, timeout=10) # type: ignore[assignment]
        ...
        security_logger.warning(f"Falha de autenticacao SMTP (provedor rejeitou) para {user}: {e}", exc_info=True)
        ...
        app_logger.warning(f"Falha de conectividade SMTP para {user} em {host}:{port}: {e}", exc_info=True)
        ...
        app_logger.error(f"Erro inesperado no teste SMTP para {user}: {e}", exc_info=True)
...
def send_email(subject, body_html, recipients, smtp_settings, plain_password, from_name=None, reply_to=None):
    ...
    try:
        ...
        app_logger.error(f"Falha ao enviar e-mail real por {user}: {e}", exc_info=True)
...
def send_email_with_credentials(
    ...
    try:
        ...
        if use_ssl:
            ...
            server = smtplib.SMTP(host, int(port), timeout=timeout) # type: ignore[assignment]
        ...
        app_logger.error(f"Falha ao enviar e-mail com credenciais para {to_email}: {e}", exc_info=True)
...
def send_email_global(subject, body_html, recipients, from_name=None, reply_to=None, body_text=None):
    ...
    if driver == "sendgrid":
        ...
        if body_text:
            ...
            try:
                ...
                app_logger.warning(f"Falha ao gerar texto plano a partir do HTML para e-mail SendGrid: {e}", exc_info=True)
        ...
        try:
            ...
            app_logger.warning(f"Falha ao configurar tracking settings do SendGrid: {e}", exc_info=True)
        ...
        try:
            ...
            app_logger.error(f"Falha de rede no envio via SendGrid: {e}", exc_info=True)
    ...
    try:
        ...
        app_logger.error(f"Falha de autenticação SMTP: {e}", exc_info=True)

        raise

    except (OSError, socket.gaierror, smtplib.SMTPConnectError, smtplib.SMTPServerDisconnected, TimeoutError) as e:

        app_logger.error(f"Falha de conectividade SMTP ({host}:{port}): {e}", exc_info=True)

        raise

    except Exception as e:

        app_logger.error(f"Falha no envio de e-mail global (SMTP): {e}", exc_info=True)
...
Este é um email automático do sistema CS Onboarding.
    ...
    def _send_email_background():
        ...
        with current_app.app_context():
            ...
            try:
                ...
                app_logger.error(f"Falha ao enviar notificação de comentário externo para {to_email}: {e}", exc_info=True)
    ...
    app = getattr(flask, 'current_app')._get_current_object() # type: ignore[attr-defined]
