...
class ChecklistRenderer {
    ...
    async loadTags() {
        ...
        try {
            ...
            const tags = await configService.getTags('comentario');
            this.tagsList = tags;
            this.tagsMap = {};
            this.tagsData = {};
            tags.forEach(t => {
    ...
    renderTagOptions(itemId) {
        if (!this.tagsList || this.tagsList.length === 0) {
            const fallbackTags = ["Ação interna", "Reunião", "No Show", "Simples registro", "Visita", "Live"];
            return [
