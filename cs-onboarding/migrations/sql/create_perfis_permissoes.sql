...
-- Inserir Perfis Padrão
WITH novos_perfis (nome, descricao, sistema, cor, icone, criado_por) AS (
  VALUES
...
SELECT n.nome, n.descricao, n.sistema, n.cor, n.icone, n.criado_por
FROM novos_perfis n
WHERE NOT EXISTS (SELECT 1 FROM perfis_acesso p WHERE p.nome = n.nome);

-- Inserir Recursos do Sistema
-- Inserir Recursos do Sistema
WITH novos_recursos (codigo, nome, descricao, categoria, tipo, ordem) AS (
  VALUES
    -- Dashboard
...
SELECT n.codigo, n.nome, n.descricao, n.categoria, n.tipo, n.ordem
FROM novos_recursos n
WHERE NOT EXISTS (SELECT 1 FROM recursos r WHERE r.codigo = n.codigo);
...
WHERE NOT EXISTS (
    ...
    WHERE p.perfil_id = (SELECT id FROM perfis_acesso WHERE nome = 'Administrador') 
    AND p.recurso_id = r.id
);
...
AND NOT EXISTS (
    SELECT 1 FROM permissoes p 
    WHERE p.perfil_id = (SELECT id FROM perfis_acesso WHERE nome = 'Implantador') 
    AND p.recurso_id = r.id
);
...
AND NOT EXISTS (
    SELECT 1 FROM permissoes p 
    WHERE p.perfil_id = (SELECT id FROM perfis_acesso WHERE nome = 'Visualizador') 
    AND p.recurso_id = r.id
);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_permissoes_perfil' AND n.nspname = 'public') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_permissoes_recurso' AND n.nspname = 'public') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_recursos_categoria' AND n.nspname = 'public') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_recursos_codigo' AND n.nspname = 'public') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_usuarios_perfil' AND n.nspname = 'public') THEN
