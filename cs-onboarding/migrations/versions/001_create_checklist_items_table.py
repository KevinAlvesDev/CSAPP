...
def upgrade():
    ...
    if dialect == 'postgresql':
        ...
        op.execute(text("""
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_parent_id' AND n.nspname = 'public') THEN
                    CREATE INDEX idx_checklist_parent_id ON checklist_items(parent_id);
                END IF;
            END$$;
        """))
        ...
        op.execute(text("""
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_implantacao_id' AND n.nspname = 'public') THEN
                    CREATE INDEX idx_checklist_implantacao_id ON checklist_items(implantacao_id);
                END IF;
            END$$;
        """))
        ...
        op.execute(text("""
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_completed' AND n.nspname = 'public') THEN
        ...
        op.execute(text("""
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_ordem' AND n.nspname = 'public') THEN
                    CREATE INDEX idx_checklist_ordem ON checklist_items(ordem);
                END IF;
            END$$;
        """))
        ...
        op.execute(text("""
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_parent_ordem' AND n.nspname = 'public') THEN
                    CREATE INDEX idx_checklist_parent_ordem ON checklist_items(parent_id, ordem);
                END IF;
            END$$;
        """))
