    ...
    if dialect == 'postgresql':
        ...
        if not cursor.fetchone():
            ...
            op.execute(text("""
                DO $$
                BEGIN
                    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_comentarios_h_checklist_item_id' AND n.nspname = 'public') THEN
                        CREATE INDEX idx_comentarios_h_checklist_item_id ON comentarios_h(checklist_item_id);
                    END IF;
                END$$;
            """))
            print("Coluna checklist_item_id adicionada em comentarios_h")
        else:
            print("Coluna checklist_item_id já existe em comentarios_h")
