...
def upgrade_postgres(cursor):
    ...
    cursor.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_fin_items_impl' AND n.nspname = 'public') THEN
                CREATE INDEX idx_checklist_fin_items_impl ON checklist_finalizacao_items(implantacao_id);
            END IF;
        END$$;
    """)
    ...
    cursor.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_fin_items_concluido' AND n.nspname = 'public') THEN
                CREATE INDEX idx_checklist_fin_items_concluido ON checklist_finalizacao_items(implantacao_id, concluido);
            END IF;
        END$$;
    """)
