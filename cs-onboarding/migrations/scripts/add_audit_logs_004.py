...
def upgrade_postgres(cursor):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS audit_logs (
            ...
            changes JSON,                -- Mudanças {before: ..., after: ...}
            metadata JSON,               -- Dados extras (razão, contexto)
    ...
    cursor.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_audit_user_email' AND n.nspname = 'public') THEN
                CREATE INDEX idx_audit_user_email ON audit_logs(user_email);
            END IF;
            
            IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_audit_action' AND n.nspname = 'public') THEN
                CREATE INDEX idx_audit_action ON audit_logs(action);
            END IF;
            
            IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_audit_target' AND n.nspname = 'public') THEN
                CREATE INDEX idx_audit_target ON audit_logs(target_type, target_id);
            END IF;
            
            IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_audit_created_at' AND n.nspname = 'public') THEN
