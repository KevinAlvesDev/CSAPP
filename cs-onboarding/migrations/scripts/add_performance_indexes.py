        ...
        if table_exists("checklist_items"):
            ...
            if db_type == "postgres":
                cursor.execute("""
                    DO $$
                    BEGIN
                        IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_impl_completed' AND n.nspname = 'public') THEN
                            CREATE INDEX idx_checklist_impl_completed ON checklist_items(implantacao_id, completed);
                        END IF;
                    END$$;
                """)
            else:
        ...
        if table_exists("checklist_items"):
            ...
            if db_type == "postgres":
                cursor.execute("""
                    DO $$
                    BEGIN
                        IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_parent_ordem' AND n.nspname = 'public') THEN
                            CREATE INDEX idx_checklist_parent_ordem ON checklist_items(parent_id, ordem, id) WHERE parent_id IS NOT NULL;
                        END IF;
                    END$$;
                """)
            else:
        ...
        if table_exists("implantacoes"):
            ...
            if db_type == "postgres":
                cursor.execute("""
                    DO $$
                    BEGIN
                        IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_implantacoes_user_status' AND n.nspname = 'public') THEN
                            CREATE INDEX idx_implantacoes_user_status ON implantacoes(usuario_cs, status, data_criacao);
                        END IF;
                    END$$;
                """)
            else:
        ...
        if table_exists("comentarios_h"):
            ...
            if db_type == "postgres":
                cursor.execute("""
                    DO $$
                    BEGIN
                        IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_comentarios_item_data' AND n.nspname = 'public') THEN
                            CREATE INDEX idx_comentarios_item_data ON comentarios_h(checklist_item_id, data_criacao DESC);
                        END IF;
                    END$$;
                """)
            else:
        ...
        if table_exists("timeline_log"):
            ...
            if db_type == "postgres":
                cursor.execute("""
                    DO $$
                    BEGIN
                        IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_timeline_impl_data' AND n.nspname = 'public') THEN
                            CREATE INDEX idx_timeline_impl_data ON timeline_log(implantacao_id, data_criacao DESC);
                        END IF;
                    END$$;
                """)
            else:
        ...
        if table_exists("perfil_usuario"):
            ...
            if db_type == "postgres":
                cursor.execute("""
                    DO $$
                    BEGIN
                        IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_perfil_usuario' AND n.nspname = 'public') THEN
                            CREATE INDEX idx_perfil_usuario ON perfil_usuario(usuario);
                        END IF;
                    END$$;
                """)
            else:
        ...
        if table_exists("checklist_items"):
            ...
            if db_type == "postgres":
                cursor.execute("""
                    DO $$
                    BEGIN
                        IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_checklist_tipo' AND n.nspname = 'public') THEN
                            CREATE INDEX idx_checklist_tipo ON checklist_items(tipo_item, implantacao_id, completed) WHERE tipo_item IS NOT NULL;
                        END IF;
                    END$$;
                """)
            else:
