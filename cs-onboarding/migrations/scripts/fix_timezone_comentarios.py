    ...
    try:
        ...
        if is_postgres:
            ...
            cursor.execute("""
                DO $$
                BEGIN
                    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'comentarios_h' AND column_name = 'data_criacao_tz') THEN
                        ALTER TABLE comentarios_h ADD COLUMN data_criacao_tz TIMESTAMP WITH TIME ZONE;
                    END IF;
                END$$;
            """)
